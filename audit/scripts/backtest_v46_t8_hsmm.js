#!/usr/bin/env node
'use strict';
// V46 T8 — HSMM (Hidden Semi-Markov Model) regime detector
// Yu 2010. Mejora sobre HMM: dwell-time distribution explícita previene
// transitions espureas por noise.
//
// Implementación simplificada (3 estados: LOW_VOL, MID_VOL, HIGH_VOL):
//   - Compute realized vol BTC rolling 24h (proxy global de régimen)
//   - Estados via percentiles P30/P70 sobre 30d trailing window
//   - Dwell-time: minimum 6h en estado nuevo antes de declarar transition
//   - Sizing modulation:
//     * LOW_VOL: size *= 1.20 (favorable mean-reversion)
//     * MID_VOL: size *= 1.00 (baseline)
//     * HIGH_VOL: size *= 0.65 (high uncertainty)
//
// Justificación: funding carry rinde mejor en bajo-vol, sufre en high-vol.
// HSMM con dwell-time evita whipsaw vs HMM puro.
...
console.log(`V46 T8 HSMM — ${CFG.toUpperCase()}`);
console.log('═'.repeat(80));
...
// === T8 HSMM Regime Detection ===
// Pre-compute BTC realized vol (24h rolling) as global regime indicator
...
// Determine regime per timestamp (with dwell-time enforcement)
// Regime: 0=LOW_VOL, 1=MID_VOL, 2=HIGH_VOL
const DWELL_MIN_H = 6;
const REGIME_WINDOW_H = 30 * 24;  // 30d for percentiles
const regimeAt = new Int8Array(btcBars.length).fill(1);  // default MID
let lastRegime = 1, regimeStartIdx = 0;

for(let i = REGIME_WINDOW_H; i < btcBars.length; i++){
  // Compute P30/P70 of recent 30d realized vol
  const window = [];
  for(let j = i - REGIME_WINDOW_H; j < i; j++){
    if(btcRV[j] > 0) window.push(btcRV[j]);
  }
  ...
  const rv = btcRV[i];
  let candidate;
  if(rv < p30) candidate = 0;       // LOW_VOL
  else if(rv > p70) candidate = 2;  // HIGH_VOL
  else candidate = 1;               // MID_VOL

  // Dwell-time enforcement: only switch if at least DWELL_MIN_H hours in current
  if(candidate !== lastRegime){
    const elapsed = i - regimeStartIdx;
    if(elapsed >= DWELL_MIN_H){
      // Allow transition
      lastRegime = candidate;
      regimeStartIdx = i;
    }
    // Else: stay in lastRegime
  }
  regimeAt[i] = lastRegime;
}

// Lookup: timestamp → regime via BTC bar index
function regimeAtTs(ts){
  // Binary search BTCUSDT bars
...
function t8SizeMult(ts){
  ...
  if(r === 0) return 1.20;  // LOW_VOL
  if(r === 2) return 0.65;  // HIGH_VOL
  return 1.00;              // MID_VOL
}
...
const t0=Date.now();
const regimeStats = { 0:0, 1:0, 2:0 };  // count of trades per regime
...
for(const evt of stream){
  ...
  if(FLAGS.T8){
    const reg = regimeAtTs(t);
    regimeStats[reg]++;
    sizeMult *= t8SizeMult(t);
  }
...
console.log(`Regime distribution: LOW=${regimeStats[0]} MID=${regimeStats[1]} HIGH=${regimeStats[2]}`);
...
const out = {
  ...
  regimeStats, runtime_s: parseFloat(dt)
};
fs.writeFileSync(path.join(OUT_DIR, `v46_t8_${CFG}.json`), JSON.stringify(out, null, 2));
