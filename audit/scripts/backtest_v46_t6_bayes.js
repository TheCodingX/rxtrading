#!/usr/bin/env node
'use strict';
// V46 T6 — Bayesian Hierarchical Shrinkage (Empirical Bayes)
// Stan/PyMC concept aplicado sin MCMC. Cada par tiene su propia WR,
// pero pares con poca data shrinks hacia el global mean.
//
// posterior_WR_pair = (n_pair * obs_WR_pair + n_global * obs_WR_global) / (n_pair + n_global)
//
// Aplicación:
//   - Calcular rolling WR per pair (last 30 trades)
//   - Calcular rolling WR global (last 200 trades all pairs)
//   - Compute shrunken posterior per pair
//   - Sizing modulation: posterior_WR > 0.72: size *= 1.25
//                        posterior_WR < 0.62: size *= 0.70
//                        else: 1.0
//
// Diferencia vs P11: P11 usa rolling PF crudo. T6 shrinks toward global → más estable.
...
console.log(`V46 T6 BAYESIAN — ${CFG.toUpperCase()}`);
console.log('═'.repeat(80));
...
// Tracker: per-pair last 30 outcomes, global last 200
...
function bayesPosteriorWR(pair){
  ...
  // Empirical Bayes posterior: weight by sample size
  // posterior = (pa_wins + gl_wins * (pa_n / gl_n)) / (pa_n + pa_n)  // naive
  // Better: shrinkage proportional to confidence
  // posterior_WR = (alpha + wins) / (alpha + beta + n) with prior from global
  const prior_alpha = gl_wins * 0.10;  // prior strength = 10% of global obs
...
fs.writeFileSync(path.join(OUT_DIR, `v46_t6_${CFG}.json`), JSON.stringify(out, null, 2));
