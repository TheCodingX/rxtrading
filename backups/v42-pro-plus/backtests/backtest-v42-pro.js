#!/usr/bin/env node
'use strict';
// v42 PRO Portfolio Backtest — V41 stack + filtered universe + safety limits simulation
// Tests 3 universes:
//   STRICT_12: 12 pairs PF>=1.05 n>=30 DD<=70
//   TOP_15: extend to top-15 by PF (some higher DD)
//   V41_30: full 30 pairs (baseline comparison)

const fs=require('fs');const path=require('path');

const UNIVERSES={
  V41_30:['BTCUSDT','ETHUSDT','SOLUSDT','BNBUSDT','XRPUSDT','ADAUSDT','AVAXUSDT','DOGEUSDT','LINKUSDT','LTCUSDT','DOTUSDT','ATOMUSDT','UNIUSDT','TRXUSDT','NEARUSDT','APTUSDT','POLUSDT','FILUSDT','ARBUSDT','OPUSDT','INJUSDT','SUIUSDT','TIAUSDT','SEIUSDT','RUNEUSDT','1000PEPEUSDT','WLDUSDT','FETUSDT','RENDERUSDT','JUPUSDT'],
  STRICT_12:['ADAUSDT','RENDERUSDT','ARBUSDT','ETHUSDT','XRPUSDT','BTCUSDT','1000PEPEUSDT','ATOMUSDT','LINKUSDT','POLUSDT','SUIUSDT','TRXUSDT'],
  TOP_15:['ADAUSDT','RENDERUSDT','ARBUSDT','ETHUSDT','XRPUSDT','BTCUSDT','1000PEPEUSDT','ATOMUSDT','LINKUSDT','POLUSDT','SOLUSDT','SUIUSDT','TRXUSDT','NEARUSDT','INJUSDT']
};
...
// V41 engine: ToD + adaptive + tiered wide/conservative + cluster
function runV41(signals,parsed,cfg){
  const SKIP=cfg.tod?new Set([0,7,10,11,12,13,17,18]):null;
  const MAXP=cfg.maxPos||4,MAXCL=cfg.maxCluster||3;
  const tierThr=cfg.tierThr||[5,15,30];const tierMul=cfg.tierMul||[0.3,1.0,1.2,1.5];
  ...
  // Safety state
  const safety={dailyLoss:0,dailyDate:null,consecLosses:0,cooldownUntil:0,peakCap:INIT_CAP};
  ...
  for(const sig of signals){
    ...
    if(cfg.safety){
      const now=pd.t[eb];
      // Cooldown active?
      if(now<safety.cooldownUntil)continue;
      // Daily loss stop -5%
      const todayDate=new Date(now).toISOString().slice(0,10);
      if(safety.dailyDate===todayDate&&safety.dailyLoss>0.05*(INIT_CAP+cap-INIT_CAP))continue; // simplified: 5% of current equity
      // DD stop <75% of peak
      if(cap<0.75*safety.peakCap){safety.cooldownUntil=now+6*3600000;continue;}
      // Circuit breaker: 5 consec losses -> 6h
      if(safety.consecLosses>=5){safety.cooldownUntil=now+6*3600000;safety.consecLosses=0;continue;}
      // Min balance floor $100
      if(cap<100)continue;
      // Max capital deployed 50%
      let deployed=0;for(const s of slots)if(s)deployed+=s.sz;
      if(deployed>0.5*(INIT_CAP*10))continue; // with 5x leverage, deployed cap in USD
    }
...
async function runUniverse(name,PAIRS,cfg){
  const allData={};
  ...
  const r=runV41(signals,parsed,cfg);
  const s=st(r.trades);
  const ddP=(s.mdd/(INIT_CAP+s.pnl))*100;
  const tpd=s.n/150;
  console.log(`${name.padEnd(14)} ${PAIRS.length}p  ${String(signals.length).padStart(5)}sig  ${String(s.n).padStart(4)}t  PF${s.pf.toFixed(2)} WR${s.wr.toFixed(1)}% DD${ddP.toFixed(0)}% Sh${s.sharpe.toFixed(1)} $${s.pnl.toFixed(0).padStart(5)} t/d${tpd.toFixed(2)}`);
  return{name,pairs:PAIRS.length,signals:signals.length,s,ddP,tpd};
}

async function main(){
  console.log('v42 PRO Portfolio Backtest — comparative: V41(30) / STRICT_12 / TOP_15');
  console.log('═'.repeat(100));
  const cfgV41={tod:true,adaptive:true,maxPos:4,maxCluster:3,safety:false};
  const cfgV42={tod:true,adaptive:true,maxPos:4,maxCluster:3,safety:true};
  console.log('Engine       pairs  signals  trades  PF    WR     DD   Sh    PnL      t/d');
  console.log('-'.repeat(100));
  const r1=await runUniverse('v41@30',UNIVERSES.V41_30,cfgV41);
  const r2=await runUniverse('v42PRO-12',UNIVERSES.STRICT_12,cfgV42);
  const r3=await runUniverse('v42PRO-15',UNIVERSES.TOP_15,cfgV42);
  const r4=await runUniverse('v42PRO-12ns',UNIVERSES.STRICT_12,{...cfgV42,safety:false});
  const r5=await runUniverse('v42PRO-15ns',UNIVERSES.TOP_15,{...cfgV42,safety:false});
  console.log('-'.repeat(100));
  // Delta table
  console.log('\n── DELTAS vs v41@30 (BASELINE) ──');
  const b=r1;
  for(const r of [r2,r3,r4,r5]){
    const dPF=(r.s.pf-b.s.pf).toFixed(2);const dWR=(r.s.wr-b.s.wr).toFixed(1);const dDD=(r.ddP-b.ddP).toFixed(0);const dPnL=(r.s.pnl-b.s.pnl).toFixed(0);const dTPD=(r.tpd-b.tpd).toFixed(2);
    console.log(`  ${r.name}: ΔPF${dPF>=0?'+':''}${dPF} ΔWR${dWR>=0?'+':''}${dWR}pp ΔDD${dDD>=0?'+':''}${dDD}pp ΔSh${(r.s.sharpe-b.s.sharpe).toFixed(1)} ΔPnL$${dPnL>=0?'+':''}${dPnL} Δt/d${dTPD>=0?'+':''}${dTPD}`);
  }
  // Write best config
  const candidates=[r2,r3,r4,r5].sort((a,b)=>b.s.sharpe-a.s.sharpe);
  const best=candidates[0];
  console.log(`\n🏆 BEST v42 PRO: ${best.name} (Sharpe ${best.s.sharpe.toFixed(1)})`);
  fs.writeFileSync('/tmp/v42-pro-result.json',JSON.stringify({candidates,best:best.name},null,2));
}
