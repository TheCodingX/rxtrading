...
async function main(){
  const B='═'.repeat(72);
  console.log(`\n${B}\n  APEX V18 — CONSTRAINED: 8/8pairs+7/7months then maxPF: Premium Index + Funding + ML\n${B}`);
  ...
  const slMs=[1.5,1.8,2.0,2.2,2.5,3.0],tpRs=[2.5,2.7,3.0,3.3,3.5,4.0,5.0],fwds=[1,2,3];
  const thrPs=[55,58,60,62,65,68,70,73],maxPoss=[2,3,4],timeouts=[48,60,72,84],adxFs=[0,18,20,22];
  let bestPF=0,bestCfg=null,bestTr=[];

  console.log('  Sampling 3000 CONSTRAINED configs with institutional data...\n');
  for(let att=0;att<3000;att++){
    const cfg={slM:pick(slMs),tpR:pick(tpRs),fwd:pick(fwds),thrP:pick(thrPs),maxPos:pick(maxPoss),timeout:pick(timeouts),adxF:pick(adxFs),mc:0.003+rng()*0.012};
    ...
    // HARD CONSTRAINTS: 8/8 pairs profitable AND 6+ months profitable
    const _pp={};for(const _t of allOOS){if(!_pp[_t.pair])_pp[_t.pair]=[];_pp[_t.pair].push(_t);}
    let _npp=0;for(const _p of Object.keys(_pp)){const _s2=st(_pp[_p]);if(_s2.pnl>=0)_npp++;}
    const _mo={};for(const _t of allOOS){const _m=_t.date?.slice(0,7)||'?';if(!_mo[_m])_mo[_m]=[];_mo[_m].push(_t);}
    let _npm=0;for(const _m of Object.keys(_mo)){const _s2=st(_mo[_m]);if(_s2.pnl>=0)_npm++;}
    const _nmo=Object.keys(_mo).length;
    // Hard filter: need 7+ pairs profitable, 5+ months profitable (relaxed), 50+ trades
    if(_npp>=7&&_npm>=Math.max(_nmo-1,4)&&s.n>=50&&s.pf>bestPF){bestPF=s.pf;bestCfg=cfg;bestTr=allOOS;
