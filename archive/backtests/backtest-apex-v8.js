#!/usr/bin/env node
'use strict';
// APEX V8 — ULTRA-FOCUSED around PF 1.31 zone around V6 best + ADX Filter + Multi-Position + Massive Grid Search
...
async function main(){
  const B='═'.repeat(72);
  console.log(`\n${B}\n  APEX V8 — ULTRA-FOCUSED around PF 1.31 zone around V6 best + ADX Filter + Multi-Pos + Massive Grid\n${B}`);
  ...
  const slMs=[0.9,1.0,1.05,1.1,1.15,1.2,1.3];
  const tpRs=[2.5,2.7,3.0,3.2,3.5,4.0];
  const fwds=[3,4,5];
  const thrPs=[55,56,58,60,62,65];
  const trails=[false];
  const maxPoss=[2,3,4];
  const timeouts=[24,28,30,32,36];
  const adxFilters=[18,20,22,25]; // 0=no filter
  ...
  console.log('  Sampling ~800 ultra-focused configs from grid...\n');
  ...
  for(let attempt=0;attempt<800;attempt++){
    const cfg={slM:pick(slMs),tpR:pick(tpRs),fwd:pick(fwds),thrP:pick(thrPs),
      ...
      mc:0.007+rng()*0.006}; // random minCorr 0.008-0.020
