#!/usr/bin/env node
'use strict';
// Mejora 7: Adaptive stops by volatility regime
// Low vol (ATR% < p30): SL=1.5×ATR, TP=2.5×ATR
// Mid vol: SL=2×ATR, TP=3.25×ATR (V40 current)
// High vol (ATR% > p70): SL=2.5×ATR, TP=3.5×ATR
const fs=require('fs');
...
const SKIP_HOURS=new Set([0,7,10,11,12,13,17,18]);
...
async function genSignals(allData){
  const signals=[];const parsed={};const volPctiles={};
  ...
  for(let w=0;w<nW;w++){
    ...
    for(const pair of PAIRS){if(!allData[pair])continue;const tB=allData[pair].b1h.filter(b=>b.t>=trs&&b.t<tre);if(tB.length<300)continue;const Fm=momFeatures(tB);const Fr=revFeatures(tB);const fwd=new Float64Array(Fm.n).fill(NaN);for(let i=50;i<Fm.n-2;i++)fwd[i]=(Fm.c[i+2]-Fm.c[i])/Fm.c[i]*100;const coM=pearson(Fm.fs,fwd,50,Fm.n-2);const coR=pearson(Fr.fs,fwd,50,Fr.n-2);const selM=coM.map((c,i)=>({idx:i,corr:c,abs:Math.abs(c)})).filter(x=>x.abs>=0.011).sort((a,b)=>b.abs-a.abs).slice(0,6);const selR=coR.map((c,i)=>({idx:i,corr:c,abs:Math.abs(c)})).filter(x=>x.abs>=0.011).sort((a,b)=>b.abs-a.abs).slice(0,6);if(selM.length<2)continue;let tcM=[];for(let i=55;i<Fm.n;i++){if(Fm.adx[i]<22)continue;let comp=0;for(const{idx,corr}of selM)comp+=corr*Fm.fs[idx][i];tcM.push(Math.abs(comp));}tcM.sort((a,b)=>a-b);let tcR=[];for(let i=55;i<Fr.n;i++){if(Fr.adx[i]<22)continue;let comp=0;for(const{idx,corr}of selR)comp+=corr*Fr.fs[idx][i];tcR.push(Math.abs(comp));}tcR.sort((a,b)=>a-b);const thrM=tcM[Math.floor(tcM.length*0.55)]||0.001;const thrR=tcR[Math.floor(tcR.length*0.55)]||0.001;
      // Also compute vol percentiles from training atr/price
      const volPcts=[];for(let i=50;i<Fm.n;i++)volPcts.push(Fm.atr[i]/Fm.c[i]*100);volPcts.sort((a,b)=>a-b);
      pm[pair]={selM,selR,thrM,thrR,volP30:volPcts[Math.floor(volPcts.length*0.3)],volP70:volPcts[Math.floor(volPcts.length*0.7)]};
    }
    if(Object.keys(pm).length<8)continue;
    for(const pair of PAIRS){if(!pm[pair])continue;const teB=allData[pair].b1h.filter(b=>b.t>=tes&&b.t<tee);if(teB.length<50)continue;const te4=allData[pair].b4h.filter(b=>b.t>=tes-4*3600000&&b.t<tee);const Fm=momFeatures(teB);const Fr=revFeatures(teB);if(!parsed[pair])parsed[pair]={t:[...Fm.t],o:[...Fm.o],h:[...Fm.h],l:[...Fm.l],c:[...Fm.c],atr:[...Fm.atr]};else{for(let i=0;i<Fm.t.length;i++){parsed[pair].t.push(Fm.t[i]);parsed[pair].o.push(Fm.o[i]);parsed[pair].h.push(Fm.h[i]);parsed[pair].l.push(Fm.l[i]);parsed[pair].c.push(Fm.c[i]);parsed[pair].atr.push(Fm.atr[i]);}}const{selM,selR,thrM,thrR,volP30,volP70}=pm[pair];volPctiles[pair]={volP30,volP70};const trend4=compute4HTrend(te4);const t4=te4.map(b=>b.t);let last=-3;for(let i=55;i<Fm.n-60-1;i++){if(i-last<2)continue;if(Fm.adx[i]<22)continue;let compM=0;for(const{idx,corr}of selM)compM+=corr*Fm.fs[idx][i];let compR=0;for(const{idx,corr}of selR)compR+=corr*Fr.fs[idx][i];const passM=Math.abs(compM)>=thrM;const passR=Math.abs(compR)>=thrR;const dirM=compM>0?1:-1;const dirR=-1*(compR>0?1:-1);let finalDir=0,absComp=0,thrUsed=0;if(passM&&passR&&dirM===dirR){finalDir=dirM;absComp=Math.max(Math.abs(compM),Math.abs(compR));thrUsed=Math.max(thrM,thrR);}else if(passM){finalDir=dirM;absComp=Math.abs(compM);thrUsed=thrM;}else if(passR){finalDir=dirR;absComp=Math.abs(compR);thrUsed=thrR;}if(finalDir===0)continue;const b4=findPrev(t4,Fm.t[i]);if(b4<0||trend4[b4]!==finalDir)continue;const hr=new Date(Fm.t[i]).getUTCHours();if(SKIP_HOURS.has(hr))continue;const absIdx=parsed[pair].t.length-Fm.n+i;const confRatio=absComp/thrUsed;const atrPct=Fm.atr[i]/Fm.c[i]*100;signals.push({pair,ts:Fm.t[i],dir:finalDir,absIdx,atr:Fm.atr[i],atrPct,confRatio,hr});last=i;}}
  }
  signals.sort((a,b)=>a.ts-b.ts);
  return{signals,parsed,volPctiles};
}

function runAdaptive(signals,parsed,volPctiles,slTp){
  const tierThr=[5,15,30],tierMul=[0.3,1.0,1.2,1.5];
  const maxPos=4,maxCluster=3;
  let cap=INIT_CAP;const trades=[];const slots=new Array(maxPos).fill(null);
  const prng=P(SEED);
  function cls(si,j,reason){const pos=slots[si];const pd=parsed[pos.pair];let ep2;if(reason==='SL')ep2=pos.dir===1?pos.slP*(1-SLIP_SL):pos.slP*(1+SLIP_SL);else if(reason==='TP')ep2=pos.tpP;else ep2=pd.c[j];const g=pos.dir===1?(ep2-pos.ep)*pos.qty:(pos.ep-ep2)*pos.qty;const fE=pos.sz*FEE_E,fX=pos.sz*(reason==='TP'?FEE_TP:FEE_SL);cap+=g-fE-fX;trades.push({pnl:g-fE-fX,type:reason,pair:pos.pair,date:new Date(pd.t[j]).toISOString().slice(0,10),regime:pos.regime});slots[si]=null;}
  ...
  for(const sig of signals){if(cap<=0)break;const pd=parsed[sig.pair];const eb=sig.absIdx+1;if(eb>=pd.c.length)continue;advance(pd.t[eb]);
    ...
    const cl=CLUSTERS[sig.pair]||'o';if((clCnts[cl]||0)>=maxCluster)continue;
    ...
    const vp=volPctiles[sig.pair]||{volP30:1,volP70:2};
    let regime='mid',slM=slTp.midSL,tpR=slTp.midTP;
    if(sig.atrPct<vp.volP30){regime='low';slM=slTp.lowSL;tpR=slTp.lowTP;}
    else if(sig.atrPct>vp.volP70){regime='high';slM=slTp.highSL;tpR=slTp.highTP;}
    const slPct=Math.max(0.003,Math.min(0.03,ap*slM));const tpPct=Math.max(0.005,Math.min(0.08,slPct*tpR/slM)); // tpR as ratio of ATR multiplier
    // Fix: tpR in config is TP/SL ratio
    const tpPctFinal=Math.max(0.005,Math.min(0.08,slPct*tpR));
    const pctAbove=(sig.confRatio-1)*100;
    let mult=tierMul[3];
    if(pctAbove<tierThr[0])mult=tierMul[0];
    else if(pctAbove<tierThr[1])mult=tierMul[1];
    else if(pctAbove<tierThr[2])mult=tierMul[2];
    const sz=POS_SIZE_BASE*mult;
    slots[freeSlot]={pair:sig.pair,dir:sig.dir,ep,slP:sig.dir===1?ep*(1-slPct):ep*(1+slPct),tpP:sig.dir===1?ep*(1+tpPctFinal):ep*(1-tpPctFinal),qty:sz/ep,sz,regime,eb,exp:eb+60,nc:eb+1};
  }
...
async function main(){
  console.log('Mejora 7: Adaptive stops by volatility regime');
  ...
  const{signals,parsed,volPctiles}=await genSignals(allData);
  console.log(`Signals: ${signals.length}\n`);

  // Baseline: uniform SL=2, TP=3.25 (ratio 1.625)
  const baseCfg={lowSL:2,lowTP:3.25,midSL:2,midTP:3.25,highSL:2,highTP:3.25};
  const base=runAdaptive(signals,parsed,volPctiles,baseCfg);
  ...
  // Adaptive configs
  const configs=[
    {name:'V7-spec',lowSL:1.5,lowTP:2.5,midSL:2,midTP:3.25,highSL:2.5,highTP:3.5},
    {name:'V7-wider',lowSL:1.5,lowTP:2.0,midSL:2,midTP:3.25,highSL:3,highTP:4.0},
    {name:'V7-low-only',lowSL:1.5,lowTP:2.5,midSL:2,midTP:3.25,highSL:2,highTP:3.25},
    {name:'V7-high-only',lowSL:2,lowTP:3.25,midSL:2,midTP:3.25,highSL:2.5,highTP:3.5},
    {name:'V7-aggressive',lowSL:1.3,lowTP:2.2,midSL:2,midTP:3.25,highSL:2.8,highTP:4.0},
  ];
  const results=[];
  for(const cfg of configs){
    const r=runAdaptive(signals,parsed,volPctiles,cfg);
    const s=st(r.trades);
    const ddP=(s.mdd/(INIT_CAP+s.pnl))*100;
    results.push({cfg,s,ddP});
    const dPF=(s.pf-bs.pf).toFixed(2),dDD=(ddP-bddP).toFixed(0);
    // Breakdown by regime
    const byReg={low:[],mid:[],high:[]};for(const t of r.trades)byReg[t.regime].push(t);
    const lows=st(byReg.low),mids=st(byReg.mid),highs=st(byReg.high);
    console.log(`${cfg.name.padEnd(15)} ${s.n}t PF${s.pf.toFixed(2)}(${dPF>=0?'+':''}${dPF}) WR${s.wr.toFixed(1)}% DD${ddP.toFixed(0)}%(${dDD>=0?'+':''}${dDD}pp) Sh${s.sharpe.toFixed(1)} $${s.pnl.toFixed(0)}`);
    console.log(`   regime | low:${lows.n}t PF${lows.pf.toFixed(2)} | mid:${mids.n}t PF${mids.pf.toFixed(2)} | high:${highs.n}t PF${highs.pf.toFixed(2)}`);
  }
  const bySh=[...results].sort((a,b)=>b.s.sharpe-a.s.sharpe);
  const best=bySh[0];
  console.log(`\n── BEST: ${best.cfg.name} ──`);
  console.log(`Delta PF: ${(best.s.pf-bs.pf).toFixed(2)}, DD: ${(best.ddP-bddP).toFixed(0)}pp, Sh: ${(best.s.sharpe-bs.sharpe).toFixed(1)}, PnL: $${(best.s.pnl-bs.pnl).toFixed(0)}`);
}
