#!/usr/bin/env node
'use strict';
// Mejora 4: Time-of-Day filter — analyze performance by UTC hour, skip worst 4-6 hours
const fs=require('fs');
...
// Engine with ToD filter + tiered sizing (applies current V41 mejoras 2+5)
function runWithTodFilter(signals,parsed,skipHours){
  ...
  for(const sig of signals){if(cap<=0)break;
    if(skipHours.has(sig.hr))continue;
    const pd=parsed[sig.pair];const eb=sig.absIdx+1;if(eb>=pd.c.length)continue;advance(pd.t[eb]);
    ...
    slots[freeSlot]={pair:sig.pair,dir:sig.dir,ep,slP:sig.dir===1?ep*(1-slPct):ep*(1+slPct),tpP:sig.dir===1?ep*(1+tpPct):ep*(1-tpPct),qty:sz/ep,sz,eb,exp:eb+60,nc:eb+1,hr:sig.hr};
  }
...
async function main(){
  console.log('Mejora 4: Time-of-Day filter — hourly performance analysis + skip sweep');
  ...
  // Baseline (no ToD filter, Mejoras 2+5 on)
  const base=runWithTodFilter(signals,parsed,new Set());
  const bs=st(base.trades);
  const bddP=(bs.mdd/(INIT_CAP+bs.pnl))*100;
  console.log(`BASELINE (no ToD): ${bs.n}t PF${bs.pf.toFixed(2)} WR${bs.wr.toFixed(1)}% DD${bddP.toFixed(0)}% Sh${bs.sharpe.toFixed(1)} $${bs.pnl.toFixed(0)}`);

  // Per-hour performance on baseline
  console.log('\n── BASELINE PERFORMANCE BY UTC HOUR ──');
  const hourStats={};for(let h=0;h<24;h++)hourStats[h]=[];
  for(const t of base.trades)hourStats[t.hr].push(t);
  const hourPerf=[];
  for(let h=0;h<24;h++){const s=st(hourStats[h]);hourPerf.push({h,s,n:s.n,pf:s.pf,wr:s.wr,pnl:s.pnl});}
  // Sort to see worst/best
  const sortedByPF=[...hourPerf].filter(x=>x.n>=10).sort((a,b)=>a.pf-b.pf);
  console.log('Hour | Trades | WR    | PF    | PnL');
  for(const h of hourPerf)console.log(`  ${String(h.h).padStart(2,'0')}:00 | ${String(h.n).padStart(6)} | ${h.wr.toFixed(1).padStart(5)}% | ${h.pf.toFixed(2).padStart(5)} | $${h.pnl.toFixed(0).padStart(6)}`);
  console.log('\nWorst 6 hours by PF (min 10 trades):');
  for(const h of sortedByPF.slice(0,6))console.log(`  ${String(h.h).padStart(2,'0')}:00 PF${h.pf.toFixed(2)} WR${h.wr.toFixed(1)}% ${h.n}t $${h.pnl.toFixed(0)}`);

  // Sweep: skip N worst hours
  const skipNOptions=[2,4,6,8];
  const results=[];
  for(const skipN of skipNOptions){
    const skip=new Set(sortedByPF.slice(0,skipN).map(h=>h.h));
    const r=runWithTodFilter(signals,parsed,skip);
    const s=st(r.trades);
    const ddP=(s.mdd/(INIT_CAP+s.pnl))*100;
    const tpd=s.n/150;
    const skipList=[...skip].sort((a,b)=>a-b).join(',');
    results.push({skipN,skip,s,ddP,tpd,skipList});
    const dPF=(s.pf-bs.pf).toFixed(2),dDD=(ddP-bddP).toFixed(0);
    console.log(`\nSkip ${skipN} worst hours (${skipList}): ${s.n}t ${tpd.toFixed(2)}t/d PF${s.pf.toFixed(2)}(${dPF>=0?'+':''}${dPF}) WR${s.wr.toFixed(1)}% DD${ddP.toFixed(0)}%(${dDD>=0?'+':''}${dDD}pp) Sh${s.sharpe.toFixed(1)} $${s.pnl.toFixed(0)}`);
  }

  const bySharpe=[...results].sort((a,b)=>b.s.sharpe-a.s.sharpe);
  const best=bySharpe[0];
  console.log(`\n── BEST: Skip ${best.skipN} hours (${best.skipList}) ──`);
