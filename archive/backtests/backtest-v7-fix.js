#!/usr/bin/env node
// ══════════════════════════════════════════════════════════════
// BACKTEST v7.1 FIX — Based on v7 results
// FIX 1: Trailing stop only at 80% TP (was 50%/75%)
// FIX 2: Much longer cooldowns (VIP: 100 bars, Scalp: 30, Free: 48)
// FIX 3: Higher score thresholds (VIP: 9, Scalp: 6)
// FIX 4: Block toxic hours (VIP: 20-23 UTC, Scalp: 15-18 UTC)
// FIX 5: More reachable TP (VIP: 1.5 ATR, Scalp: 1.0 ATR)
// ══════════════════════════════════════════════════════════════
...
function genSignal(C5, H5, L5, V5, C15, H15, L15, V15, C1h, H1h, L1h, V1h, mode, hourUTC) {
  ...
  if(isStrict) {
    ...
    // FIX: Block toxic hours 20-23 UTC (WR drops to 53-56% in backtest)
    if(hourUTC >= 20 && hourUTC <= 23) { bScore -= 3; sScore -= 3; }

    // Decision — HTF gate — FIX: score threshold 9 (was 7)
    const vipMinScore = requiredScore + 2; // 9 base (12 if HTF neutral)
    if(htfTrend==='BUY'&&bScore>=vipMinScore&&bInds>=requiredInds){signal='BUY';}
    else if(htfTrend==='SELL'&&sScore>=vipMinScore&&sInds>=requiredInds){signal='SELL';}
    else if(htfTrend==='NEUTRAL'){
      if(bScore>=vipMinScore&&bInds>=requiredInds&&bScore>sScore+3)signal='BUY';
      else if(sScore>=vipMinScore&&sInds>=requiredInds&&sScore>bScore+3)signal='SELL';
    }
    ...
    // FIX: TP 1.5 ATR (was 1.8), SL 1.0 ATR (was 1.2) — more reachable
    ...
    // FIX: Block toxic hours 15-18 UTC for scalp
    if(hourUTC >= 15 && hourUTC <= 18) { bScore -= 2; sScore -= 2; }

    // FIX: score threshold 6 (was 5)
    const scalpMinScore=6;const minInds=3;
    if(scalpGate==='BUY'&&bScore>=scalpMinScore&&bInds>=minInds){signal='BUY';}
    else if(scalpGate==='SELL'&&sScore>=scalpMinScore&&sInds>=minInds){signal='SELL';}
    else if(scalpGate==='NEUTRAL'){
      if(bScore>=scalpMinScore+1&&bInds>=minInds&&bScore>sScore+2)signal='BUY';
      else if(sScore>=scalpMinScore+1&&sInds>=minInds&&sScore>bScore+2)signal='SELL';
    }
    if(signal==='NEUTRAL'){
      if(bScore>=scalpMinScore+2&&bInds>=minInds+1&&bScore>sScore+2.5)signal='BUY';
      else if(sScore>=scalpMinScore+2&&sInds>=minInds+1&&sScore>bScore+2.5)signal='SELL';
    }
    ...
    // FIX: TP 1.0 ATR (was 1.2), SL 0.7 (was 0.8) — more reachable
    tpMult=1.0;slMult=0.7;
...
function evaluateTrade(signal, entry, tp, sl, futureH, futureL, futureC, mode, maxBars) {
  // FIX: Trailing only at 80% TP → breakeven (was 50%/75%)
  // This lets winners run to full TP instead of exiting early
  ...
  for(let i = 0; i < Math.min(futureH.length, maxBars); i++) {
    ...
    // FIX: Single-stage trailing — only at 80% of TP, move to breakeven
    if(hasTrailing && !slMovedBE) {
      if(signal === 'BUY') {
        const tpDist = tp - entry;
        const bestProfit = high - entry;
        if(tpDist > 0 && bestProfit >= tpDist * 0.80) {
          const newSL = entry + entry * 0.001; // breakeven + 0.1% buffer
    ...
    // TP/SL hit check
    if(signal === 'BUY') {
      if(high >= tp) return { result: 'WIN', exitPrice: tp, bars: i + 1, trailed: slMovedBE };
      if(low <= currentSL) {
        return { result: currentSL >= entry ? 'WIN' : 'LOSS', exitPrice: currentSL, bars: i + 1, trailed: slMovedBE };
      }
    } else {
      if(low <= tp) return { result: 'WIN', exitPrice: tp, bars: i + 1, trailed: slMovedBE };
      if(high >= currentSL) {
        return { result: currentSL <= entry ? 'WIN' : 'LOSS', exitPrice: currentSL, bars: i + 1, trailed: slMovedBE };
      }
    }
...
async function runBacktest() {
  ...
  const modes = [
    // FIX: Much longer cooldowns to reduce overtrading
    // VIP: 100 bars (8.3h) → ~2.8 signals/coin/day × 20 coins = ~56/day
    // Scalp: 30 bars (2.5h) → ~9.6 signals/coin/day × 19 coins = ~182/day
    // Free: 48 bars (4h) → ~6 signals/coin/day × 4 coins = ~24/day
    { name: 'VIP INSTITUTIONAL', mode: 'strict', coins: COINS_VIP, maxBars: 48, cooldownBars: 100 },
    { name: 'SCALP', mode: 'scalp', coins: COINS_SCALP, maxBars: 12, cooldownBars: 30 },
    { name: 'FREE', mode: 'frequent', coins: COINS_FREE, maxBars: 24, cooldownBars: 48 }
  ];
