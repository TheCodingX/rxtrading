#!/usr/bin/env node
'use strict';
// APEX V21 — HEDGE MODE: Long+Short simultaneously on different pairs
// Key insight: correlation kills multi-position PF. Solution: HEDGE.
// Run 2 independent "slots": one for LONGS, one for SHORTS.
// When market drops: SHORT slot wins, LONG slot loses → net neutral.
// Edge comes from ML selecting WHICH pair to long and WHICH to short.
...
// ★ HEDGE ENGINE: separate long slot + short slot + independent directional slots
function engine(sigs,parsed,pr,cfg){
  const{slM,tpR,maxLong,maxShort,smartTO,fullTO}=cfg;
  ...
  function advAll(maxT){for(let p=positions.length-1;p>=0;p--){const p2=positions[p],pd=parsed[p2.pair];let closed=false;
    for(let j=p2.nc;j<pd.n&&j<=p2.exp&&pd.t[j]<=maxT;j++){
      ...
      if(hS){cls(p2,pd,j,'SL');positions.splice(p,1);closed=true;break;}
      if(hT){cls(p2,pd,j,'TP');positions.splice(p,1);closed=true;break;}
      // Smart timeout: cut losers at smartTO, let winners run to fullTO
      const barsIn=j-p2.eb;
      if(barsIn>=smartTO){const curPnl=p2.dir===1?(pd.c[j]-p2.ep)*p2.qty:(p2.ep-pd.c[j])*p2.qty;
        if(curPnl<=0){cls(p2,pd,j,'TO');positions.splice(p,1);closed=true;break;}}
      p2.nc=j+1;}
    if(!closed&&p2.nc>p2.exp){cls(p2,parsed[p2.pair],Math.min(p2.exp,parsed[p2.pair].n-1),'TO');positions.splice(p,1);}}}
  for(const sig of sorted){if(cap<=0)break;const d=parsed[sig.pair];const eb=sig.bar+1;if(eb>=d.n)continue;advAll(d.t[eb]);
    // ★ HEDGE: separate limits for longs and shorts
    const nLong=positions.filter(p=>p.dir===1).length;
    const nShort=positions.filter(p=>p.dir===-1).length;
    if(sig.dir===1&&nLong>=maxLong)continue;
    if(sig.dir===-1&&nShort>=maxShort)continue;
    if(positions.some(p=>p.pair===sig.pair))continue;
    if(pr()>=0.75)continue;if(cap<50)continue;
    const ep=d.o[eb],atr2=at(d.h,d.l,d.c),ap=atr2[sig.bar]/d.c[sig.bar];if(ap<=0||isNaN(ap))continue;
    let sl=Math.max(0.003,Math.min(0.03,ap*slM)),tp=Math.max(0.005,Math.min(0.08,sl*tpR));
    positions.push({pair:sig.pair,dir:sig.dir,ep,slP:sig.dir===1?ep*(1-sl):ep*(1+sl),tpP:sig.dir===1?ep*(1+tp):ep*(1-tp),qty:POS_SIZE/ep,eb,exp:eb+fullTO,nc:eb+1,slD:sl});}
...
async function main(){
  console.log('APEX V21 — HEDGE MODE + Smart Timeout');
  console.log('Separate long/short slots to reduce correlation damage');
  console.log('Target: PF>1.8 + 3 trades/day\n');
  ...
  let bestScore=0,bestCfg=null,bestTr=[];
  for(let att=0;att<3000;att++){
    const cfg={slM:pick([1.8,2.0,2.2,2.5,3.0]),tpR:pick([2.5,3.0,3.3,3.5,4.0]),
      fwd:pick([1,2,3]),thrP:pick([58,60,63,65,68,70,73]),
      maxLong:pick([1,2]),maxShort:pick([1,2]),
      smartTO:pick([12,18,24,30]),fullTO:pick([60,72,84,96]),
      adxF:pick([18,20,22,25]),mc:0.004+rng()*0.012};
    ...
    for(let w=0;w<nW&&!skip;w++){const trs=startTs+w*STEP_D*864e5,tre=trs+TRAIN_D*864e5,tes=tre,tee=tes+TEST_D*864e5;if(tee>END_TS)break;
      const pm={};for(const pair of PAIRS){const tkl=allData[pair].kl.filter(k=>parseInt(k[0])>=trs&&parseInt(k[0])<tre);if(tkl.length<200)continue;const d=pa(tkl),{fs,adx:adxArr}=compF(d,allData[pair].fr,allData[pair].pi);const fwd=new Float64Array(d.n).fill(NaN);for(let i=50;i<d.n-cfg.fwd;i++)fwd[i]=(d.c[i+cfg.fwd]-d.c[i])/d.c[i]*100;const co=pearson(fs,fwd,50,d.n-cfg.fwd);const sel=co.map((c2,i)=>({idx:i,corr:c2,abs:Math.abs(c2)})).filter(x=>x.abs>=cfg.mc).sort((a,b)=>b.abs-a.abs).slice(0,12);if(sel.length<2)continue;let tc=[];for(let i=55;i<d.n;i++){if(cfg.adxF>0&&adxArr[i]<cfg.adxF)continue;let comp=0;for(const{idx,corr}of sel)comp+=corr*fs[idx][i];tc.push(Math.abs(comp));}tc.sort((a,b)=>a-b);pm[pair]={sel,thr:tc[Math.floor(tc.length*cfg.thrP/100)]||0.001};}
      if(Object.keys(pm).length<4){skip=true;break;}
      const tSigs=[],tPar={};for(const pair of PAIRS){if(!pm[pair])continue;const tkl=allData[pair].kl.filter(k=>parseInt(k[0])>=tes&&parseInt(k[0])<tee);if(tkl.length<50)continue;const d=pa(tkl);tPar[pair]=d;const{fs,adx:adxArr}=compF(d,allData[pair].fr,allData[pair].pi);const{sel,thr}=pm[pair];let last=-2;for(let i=55;i<d.n-cfg.fullTO-1;i++){if(i-last<2)continue;if(cfg.adxF>0&&adxArr[i]<cfg.adxF)continue;let comp=0;for(const{idx,corr}of sel)comp+=corr*fs[idx][i];if(Math.abs(comp)<thr)continue;tSigs.push({bar:i,dir:comp>0?1:-1,ts:d.t[i],pair});last=i;}}
      allOOS.push(...engine(tSigs,tPar,P(SEED+w),cfg).trades);}
    if(skip||!allOOS.length)continue;const s=st(allOOS);
    const tpd=s.n/(nW*TEST_D);
    // Score: PF × min(1, tpd/2) — rewards both PF and trades/day
    const score=s.pf*Math.min(1,tpd/2);
    if(s.n>=80&&score>bestScore){bestScore=score;bestCfg=cfg;bestTr=allOOS;
      console.log(`[${att}] sl=${cfg.slM} tp=${cfg.tpR} fwd=${cfg.fwd} thr=${cfg.thrP} mxL=${cfg.maxLong} mxS=${cfg.maxShort} sTO=${cfg.smartTO} fTO=${cfg.fullTO} adx=${cfg.adxF} mc=${cfg.mc.toFixed(3)} | ${s.n}t ${tpd.toFixed(1)}t/d WR ${s.wr.toFixed(1)}% PF ${s.pf.toFixed(2)} $${s.pnl.toFixed(0)} score=${score.toFixed(2)} ◄`);}}
  if(!bestCfg){console.log('No config found.');return;}
  const s=st(bestTr);const tpd=s.n/(nW*TEST_D);
  console.log(`\n${'═'.repeat(72)}`);
  console.log(`BEST: sl=${bestCfg.slM} tp=${bestCfg.tpR} fwd=${bestCfg.fwd} thr=${bestCfg.thrP} maxL=${bestCfg.maxLong} maxS=${bestCfg.maxShort} sTO=${bestCfg.smartTO} fTO=${bestCfg.fullTO} adx=${bestCfg.adxF}`);
  console.log(`PF: ${s.pf.toFixed(2)} | WR: ${s.wr.toFixed(1)}% | $${s.pnl.toFixed(0)} | ${s.n}t (${tpd.toFixed(1)}/day) | TP:${s.tp} SL:${s.sl} TO:${s.to}`);
  ...
  let lv=s.pf>=1.8&&tpd>=2?'★★★ GOAL':s.pf>=1.6?'TARGET':s.pf>=1.3?'COMPETITIVE':'BELOW';
