#!/usr/bin/env node
'use strict';
// APEX V20 — Smart Timeout + Trailing + maxPos 2-3
// Innovation: Cut losers fast (smartTO hours), let winners run (fullTO hours)
// This frees position slots faster → more trades/day without sacrificing PF
...
// ★ SMART ENGINE: cut losers fast, let winners run, with trailing
function engine(sigs,parsed,pr,cfg){
  const{slM,tpR,maxPos,smartTO,fullTO,trail}=cfg;
...
async function main(){
  console.log('APEX V20 — Smart Timeout + Trailing + Multi-Position');
  console.log('Cut losers fast, let winners run. Target: PF>1.87 + 2-3 trades/day\n');
  ...
  console.log('Sampling 2500 configs with smart timeout...\n');
  for(let att=0;att<2500;att++){
    const cfg={slM:pick([1.8,2.0,2.2,2.5,3.0]),tpR:pick([2.5,2.7,3.0,3.3,3.5,4.0]),
      fwd:pick([1,2,3]),thrP:pick([60,63,65,68,70,73]),
      maxPos:pick([2,3]),smartTO:pick([12,18,24,30]),fullTO:pick([60,72,84,96]),
      adxF:pick([18,20,22,25]),mc:0.004+rng()*0.012,trail:pick([true,false])};
    if(cfg.smartTO>=cfg.fullTO)continue;
    ...
    // Must have 2+ trades/day AND PF > current best
    if(s.n>=100&&tpd>=1.0&&s.pf>bestPF){bestPF=s.pf;bestCfg=cfg;bestTr=allOOS;
      console.log(`[${att}] sl=${cfg.slM} tp=${cfg.tpR} fwd=${cfg.fwd} thr=${cfg.thrP} mp=${cfg.maxPos} sTO=${cfg.smartTO} fTO=${cfg.fullTO} adx=${cfg.adxF} tr=${cfg.trail?'Y':'N'} mc=${cfg.mc.toFixed(3)} | ${s.n}t ${tpd.toFixed(1)}t/d WR ${s.wr.toFixed(1)}% PF ${s.pf.toFixed(2)} $${s.pnl.toFixed(0)} ◄`);}}
  if(!bestCfg){console.log('No config found.');return;}
  const s=st(bestTr);const tpd=s.n/(nW*TEST_D);
  console.log(`\n${'═'.repeat(72)}`);
  console.log(`BEST: sl=${bestCfg.slM} tp=${bestCfg.tpR} fwd=${bestCfg.fwd} thr=${bestCfg.thrP} mp=${bestCfg.maxPos} smartTO=${bestCfg.smartTO} fullTO=${bestCfg.fullTO} adx=${bestCfg.adxF} trail=${bestCfg.trail}`);
  ...
  let lv='BELOW';if(s.pf>=1.0)lv='BREAKEVEN';if(s.pf>=1.3)lv='COMPETITIVE';if(s.pf>=1.6)lv='TARGET';if(s.pf>=1.87)lv='EXCEEDED ★★★';
  console.log(`\n${'═'.repeat(72)}\nVERDICT: PF ${s.pf.toFixed(2)} | ${tpd.toFixed(1)} trades/day | $${s.pnl.toFixed(0)} | [${lv}]`);
