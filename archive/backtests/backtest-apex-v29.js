#!/usr/bin/env node
'use strict';
// APEX V29 — V16 ARCHITECTURE + 3 NEW INSTITUTIONAL DATA STREAMS
// NEW DATA (never used in V1-V28):
//   1. Open Interest History (/futures/data/openInterestHist) — institutional position building
//   2. Top Trader Long/Short Ratio (/futures/data/topLongShortAccountRatio) — smart money positioning
//   3. Taker Buy/Sell Volume Ratio (/futures/data/takerlongshortRatio) — aggressive flow direction
// NEW FEATURES (7): OIchg1h, OIchg6h, OIchg24h, OIvsPrice, TopLSR, TopLSRslope, TakerRatio
// Same V16 per-pair ML feature selection, thresholds P40-P75 tested to find 2/day sweet spot
...
async function gOI(s,pd,st,en){let a=[],c=st;while(c<en){const u=`https://fapi.binance.com/futures/data/openInterestHist?symbol=${s}&period=${pd}&startTime=${Math.floor(c)}&endTime=${Math.floor(en)}&limit=500`;try{const k=await hg(u);if(!Array.isArray(k)||!k.length)break;a=a.concat(k);c=parseInt(k[k.length-1].timestamp)+1;}catch(e){break;}await new Promise(r=>setTimeout(r,300));}return a;}
async function gLSR(s,pd,st,en){let a=[],c=st;while(c<en){const u=`https://fapi.binance.com/futures/data/topLongShortAccountRatio?symbol=${s}&period=${pd}&startTime=${Math.floor(c)}&endTime=${Math.floor(en)}&limit=500`;try{const k=await hg(u);if(!Array.isArray(k)||!k.length)break;a=a.concat(k);c=parseInt(k[k.length-1].timestamp)+1;}catch(e){break;}await new Promise(r=>setTimeout(r,300));}return a;}
async function gTR(s,pd,st,en){let a=[],c=st;while(c<en){const u=`https://fapi.binance.com/futures/data/takerlongshortRatio?symbol=${s}&period=${pd}&startTime=${Math.floor(c)}&endTime=${Math.floor(en)}&limit=500`;try{const k=await hg(u);if(!Array.isArray(k)||!k.length)break;a=a.concat(k);c=parseInt(k[k.length-1].timestamp)+1;}catch(e){break;}await new Promise(r=>setTimeout(r,300));}return a;}
...
// Full V16 feature set + 7 NEW institutional features
function comp(d,fr,piKl,oiArr,lsrArr,trArr){
  ...
  // OI lookup
  const oiT=oiArr.map(x=>+x.timestamp),oiV=oiArr.map(x=>parseFloat(x.sumOpenInterest));
  function gOIbar(bt,lookback){let lo=0,hi=oiT.length-1,b=-1;while(lo<=hi){const m=(lo+hi)>>1;if(oiT[m]<=bt){b=m;lo=m+1;}else hi=m-1;}if(b<lookback||b<0)return 0;const cur=oiV[b],old=oiV[b-lookback];return old>0?(cur-old)/old:0;}
  function gOIval(bt){let lo=0,hi=oiT.length-1,b=-1;while(lo<=hi){const m=(lo+hi)>>1;if(oiT[m]<=bt){b=m;lo=m+1;}else hi=m-1;}return b>=0?oiV[b]:0;}
  // LSR lookup
  const lsrT=lsrArr.map(x=>+x.timestamp),lsrV=lsrArr.map(x=>parseFloat(x.longShortRatio));
  function gLSRv(bt){let lo=0,hi=lsrT.length-1,b=-1;while(lo<=hi){const m=(lo+hi)>>1;if(lsrT[m]<=bt){b=m;lo=m+1;}else hi=m-1;}return b>=0?lsrV[b]:1;}
  function gLSRslope(bt){let lo=0,hi=lsrT.length-1,b=-1;while(lo<=hi){const m=(lo+hi)>>1;if(lsrT[m]<=bt){b=m;lo=m+1;}else hi=m-1;}if(b<6)return 0;return lsrV[b]-lsrV[b-6];}
  // Taker lookup
  const trT=trArr.map(x=>+x.timestamp),trV=trArr.map(x=>parseFloat(x.buySellRatio));
  function gTRv(bt){let lo=0,hi=trT.length-1,b=-1;while(lo<=hi){const m=(lo+hi)>>1;if(trT[m]<=bt){b=m;lo=m+1;}else hi=m-1;}return b>=0?trV[b]:1;}

  const fs=[],nm=[];
  const F=(name,fn)=>{const a=new Float64Array(n);for(let i=50;i<n;i++){const v2=fn(i);a[i]=isNaN(v2)||!isFinite(v2)?0:v2;}fs.push(a);nm.push(name);};
  // V16 baseline features
  ...
  F('HLrange',i=>(h[i]-l[i])/(atr2[i]||1));
  F('BBw',i=>!isNaN(bbd.up[i])&&bbd.mid[i]?(bbd.up[i]-bbd.dn[i])/bbd.mid[i]:0);
  F('TRspike',i=>{const tr2=Math.max(h[i]-l[i],Math.abs(h[i]-c[i-1]||0),Math.abs(l[i]-c[i-1]||0));return atr2[i]>0?tr2/atr2[i]-1:0;});
  // NEW institutional features — ⭐ these are the novel addition
  F('OIchg1h',i=>gOIbar(t[i],1)*100);
  F('OIchg6h',i=>gOIbar(t[i],6)*100);
  F('OIchg24h',i=>gOIbar(t[i],24)*100);
  F('OIvsPrice',i=>{const oiC=gOIbar(t[i],6)*100;const pC=i>=6?(c[i]-c[i-6])/c[i-6]*100:0;return oiC-pC;}); // OI rising while price flat = accumulation
  F('TopLSR',i=>{const r=gLSRv(t[i]);return r>0?(1/r-1):0;}); // smart money L/S — extreme long = fade
  F('TopLSRs',i=>-gLSRslope(t[i])*10); // slope reversed = smart money shifting
  F('TakerR',i=>{const r=gTRv(t[i]);return r>0?Math.log(r):0;}); // aggressive buy flow direction
...
// V16-exact engine (maxPos=1, unchanged)
...
async function main(){
  ...
  console.log('APEX V29 — NEW INSTITUTIONAL DATA + V16 ARCHITECTURE');
  console.log('NEW: Open Interest + Top Trader L/S Ratio + Taker Buy/Sell Ratio');
  console.log('GOAL: Maintain V16 PF>=1.87 but with 2+ trades/day');
  console.log('═'.repeat(78));
  const startTs=END_TS-DAYS*864e5;const allData={};
  console.log('\nDownloading OHLCV + FR + PI + OI + LSR + TR for 8 pairs...');
  for(const pair of PAIRS){
    ...
    const oi=await gOI(pair,'1h',startTs,END_TS);
    const lsr=await gLSR(pair,'1h',startTs,END_TS);
    const tr=await gTR(pair,'1h',startTs,END_TS);
    allData[pair]={kl,fr,pi,oi,lsr,tr};
    process.stdout.write(`(k:${kl.length} oi:${oi.length} lsr:${lsr.length} tr:${tr.length}) `);
  }
  console.log('\n\nFEATURE COUNT: 32 (25 baseline + 7 NEW institutional)');
  console.log('New features: OIchg1h, OIchg6h, OIchg24h, OIvsPrice, TopLSR, TopLSRs, TakerR\n');

  const nW=Math.floor((DAYS-TRAIN_D-TEST_D)/STEP_D)+1;
  // Grid: test each threshold percentile to find 2/day sweet spot
  // Key question: does adding NEW institutional data let us LOWER threshold without losing PF?
  const cfgs=[
    // BASELINE V16 (for reference)
    {name:'V16-ref',fwd:2,thrP:65,slM:2.5,tpR:1.2,mc:0.05,adxF:25,to:60},
    // NEW DATA with progressively lower thresholds
    {name:'V29-P65',fwd:2,thrP:65,slM:2.5,tpR:1.2,mc:0.05,adxF:25,to:60},
    {name:'V29-P60',fwd:2,thrP:60,slM:2.5,tpR:1.2,mc:0.05,adxF:25,to:60},
    {name:'V29-P55',fwd:2,thrP:55,slM:2.5,tpR:1.2,mc:0.05,adxF:25,to:60},
    {name:'V29-P50',fwd:2,thrP:50,slM:2.5,tpR:1.2,mc:0.05,adxF:25,to:60},
    {name:'V29-P45',fwd:2,thrP:45,slM:2.5,tpR:1.2,mc:0.05,adxF:25,to:60},
    {name:'V29-P40',fwd:2,thrP:40,slM:2.5,tpR:1.2,mc:0.05,adxF:25,to:60},
    // Tighter SL variants at P50 and P45
    {name:'V29-P50-T',fwd:2,thrP:50,slM:2.0,tpR:1.5,mc:0.05,adxF:25,to:60},
    {name:'V29-P45-T',fwd:2,thrP:45,slM:2.0,tpR:1.5,mc:0.05,adxF:25,to:60},
    // Higher ADX filter at lower thresholds
    {name:'V29-P50-ADX',fwd:2,thrP:50,slM:2.5,tpR:1.2,mc:0.05,adxF:30,to:60},
    {name:'V29-P45-ADX',fwd:2,thrP:45,slM:2.5,tpR:1.2,mc:0.05,adxF:30,to:60},
    // Higher min-corr (only strongest features)
    {name:'V29-P45-MC',fwd:2,thrP:45,slM:2.5,tpR:1.2,mc:0.08,adxF:25,to:60},
    {name:'V29-P40-MC',fwd:2,thrP:40,slM:2.5,tpR:1.2,mc:0.08,adxF:25,to:60},
    // Fwd=1 (faster signals)
    {name:'V29-P50-F1',fwd:1,thrP:50,slM:2.5,tpR:1.2,mc:0.05,adxF:25,to:60},
    {name:'V29-P45-F1',fwd:1,thrP:45,slM:2.5,tpR:1.2,mc:0.05,adxF:25,to:60},
    // fwd=3 (slower, better correlation)
    {name:'V29-P45-F3',fwd:3,thrP:45,slM:2.5,tpR:1.2,mc:0.05,adxF:25,to:60},
  ];
  const results=[];
  for(const cfg of cfgs){
    ...
    for(let w=0;w<nW;w++){
      ...
      for(const pair of PAIRS){
        ...
        const d=pa(tkl);
        const{fs,adx:adxArr}=comp(d,allData[pair].fr,allData[pair].pi,allData[pair].oi,allData[pair].lsr,allData[pair].tr);
        ...
        const sel=co.map((c2,i)=>({idx:i,corr:c2,abs:Math.abs(c2)})).filter(x=>x.abs>=cfg.mc).sort((a,b)=>b.abs-a.abs).slice(0,10);
        if(sel.length<3)continue;
        let tc=[];
        for(let i=55;i<d.n;i++){if(adxArr[i]<cfg.adxF)continue;let comp2=0;for(const{idx,corr}of sel)comp2+=corr*fs[idx][i];tc.push(Math.abs(comp2));}
        tc.sort((a,b)=>a-b);
        pm[pair]={sel,thr:tc[Math.floor(tc.length*cfg.thrP/100)]||0.001};
      }
      ...
      for(const pair of PAIRS){
        ...
        let last=-3;
        for(let i=55;i<d.n-cfg.to-1;i++){if(i-last<3)continue;if(adxArr[i]<cfg.adxF)continue;let comp2=0;for(const{idx,corr}of sel)comp2+=corr*fs[idx][i];if(Math.abs(comp2)<thr)continue;sigs.push({bar:i,dir:comp2>0?1:-1,ts:d.t[i],pair});last=i;}
      }
    ...
    console.log(`${cfg.name.padEnd(14)} ${s.n.toString().padStart(4)}t ${tpd.toFixed(2)}t/d WR${s.wr.toFixed(1)}% PF${s.pf.toFixed(2).padStart(5)} $${s.pnl.toFixed(0).padStart(6)} P${pp}/${Object.keys(pairsStat).length} M${pm2}/${Object.keys(mos).length}`);
  }
  console.log('\n'+'═'.repeat(78));
  console.log('WINNERS BY CATEGORY:');
  console.log('═'.repeat(78));
  const valid=results.filter(r=>r.s.n>=100);
  const byPF=[...valid].sort((a,b)=>b.s.pf-a.s.pf)[0];
  const byPnl=[...valid].sort((a,b)=>b.s.pnl-a.s.pnl)[0];
  const viable=valid.filter(r=>r.tpd>=1.8&&r.s.pf>=1.5);
  const best2pd=[...valid.filter(r=>r.tpd>=1.8)].sort((a,b)=>b.s.pf-a.s.pf)[0];
  const best175=[...valid.filter(r=>r.tpd>=1.5)].sort((a,b)=>b.s.pf-a.s.pf)[0];
  if(byPF)console.log(`Highest PF:         ${byPF.cfg.name.padEnd(14)} PF${byPF.s.pf.toFixed(2)} @ ${byPF.tpd.toFixed(2)} t/d (${byPF.s.n}t $${byPF.s.pnl.toFixed(0)})`);
  if(byPnl)console.log(`Highest PnL:        ${byPnl.cfg.name.padEnd(14)} PF${byPnl.s.pf.toFixed(2)} @ ${byPnl.tpd.toFixed(2)} t/d (${byPnl.s.n}t $${byPnl.s.pnl.toFixed(0)})`);
  ...
  if(best175)console.log(`Best @ 1.5+ t/d:    ${best175.cfg.name.padEnd(14)} PF${best175.s.pf.toFixed(2)} @ ${best175.tpd.toFixed(2)} t/d (${best175.s.n}t $${best175.s.pnl.toFixed(0)})`);
  if(viable.length){
  ...
  }else{console.log('\n✗ No config achieved both 2+ t/d AND PF>=1.5');}
  if(best2pd&&best2pd.s.pf>=1.5){
    console.log('\n'+'═'.repeat(78));
    console.log(`DETAILED BREAKDOWN: ${best2pd.cfg.name}`);
    console.log('═'.repeat(78));
