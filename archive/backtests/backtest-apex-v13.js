...
async function main(){
  const B='═'.repeat(72);
  console.log(`\n${B}\n  APEX V13 — INSTITUTIONAL: Premium Index + Funding + ML\n${B}`);
  ...
  const slMs=[0.8,1.0,1.1,1.2,1.3,1.5,1.8],tpRs=[2.0,2.5,3.0,3.5,4.0,5.0],fwds=[3,4,5,6];
  const thrPs=[55,58,60,63,65,70,75],maxPoss=[1,2,3],timeouts=[24,30,36,48],adxFs=[0,18,22,25];
  let bestPF=0,bestCfg=null,bestTr=[];

  console.log('  Sampling 1000 configs with institutional data...\n');
  for(let att=0;att<1000;att++){
    const cfg={slM:pick(slMs),tpR:pick(tpRs),fwd:pick(fwds),thrP:pick(thrPs),maxPos:pick(maxPoss),timeout:pick(timeouts),adxF:pick(adxFs),mc:0.005+rng()*0.015};
