...
async function main(){
  ...
  // PARAMETRIC SWEEP: V16 fixed config, vary maxPos + timeout
  const results=[];
  for(const mp of [1,2,3,4,5,6]){
    for(const to of [24,36,48,60,72,84]){
      const cfg={slM:2,tpR:3,fwd:2,thrP:68,maxPos:mp,timeout:to,adxF:22,mc:0.011};
      const nW2=Math.floor((DAYS-TRAIN_D-TEST_D)/STEP_D)+1;const allOOS=[];let skip=false;
      for(let w=0;w<nW2&&!skip;w++){
        ...
        for(const pair of PAIRS){const tkl=allData[pair].kl.filter(k=>parseInt(k[0])>=trs&&parseInt(k[0])<tre);if(tkl.length<200)continue;
          const d=pa(tkl),{fs:feats,nm,adx:adxArr}=compF(d,allData[pair].fr,allData[pair].pi);
          const fwd=new Float64Array(d.n).fill(NaN);for(let i=50;i<d.n-cfg.fwd;i++)fwd[i]=(d.c[i+cfg.fwd]-d.c[i])/d.c[i]*100;
          const co=pearson(feats,fwd,50,d.n-cfg.fwd);
          const sel=co.map((c2,i)=>({idx:i,corr:c2,abs:Math.abs(c2)})).filter(x=>x.abs>=cfg.mc).sort((a,b)=>b.abs-a.abs).slice(0,12);
          if(sel.length<2)continue;
          let tc=[];for(let i=55;i<d.n;i++){if(cfg.adxF>0&&adxArr[i]<cfg.adxF)continue;let comp=0;for(const{idx,corr}of sel)comp+=corr*feats[idx][i];tc.push(Math.abs(comp));}
        ...
        for(const pair of PAIRS){if(!pairModels[pair])continue;
          ...
          const d=pa(tkl);tPar[pair]=d;const{fs:feats,adx:adxArr}=compF(d,allData[pair].fr,allData[pair].pi);
          ...
          for(let i=55;i<d.n-cfg.timeout-1;i++){if(i-last<2)continue;if(cfg.adxF>0&&adxArr[i]<cfg.adxF)continue;
            let comp=0;for(const{idx,corr}of sel)comp+=corr*feats[idx][i];if(Math.abs(comp)<thr)continue;
      ...
      const s=st(allOOS);
      const tpd=(s.n/(nW2*TEST_D)).toFixed(1);
      const pp2={};for(const t of allOOS){if(!pp2[t.pair])pp2[t.pair]=[];pp2[t.pair].push(t);}
      let npp=0;for(const p of Object.keys(pp2)){if(st(pp2[p]).pnl>=0)npp++;}
      const mo2={};for(const t of allOOS){const m=t.date?.slice(0,7)||'?';if(!mo2[m])mo2[m]=[];mo2[m].push(t);}
      let npm=0;for(const m of Object.keys(mo2)){if(st(mo2[m]).pnl>=0)npm++;}
      results.push({mp,to,n:s.n,tpd,wr:s.wr,pf:s.pf,pnl:s.pnl,npp,npm,nmo:Object.keys(mo2).length});
    }
  }
  console.log('
  mp | to  | Trades | t/day | WR%    | PF     | PnL$     | Mo+   | Pr+');
  console.log('  ' + '─'.repeat(75));
  for(const r of results){
    const tag=r.pf>=1.8?' ★★':r.pf>=1.5?' ★':r.pf>=1.3?' +':'';
    console.log('  '+String(r.mp).padStart(2)+' | '+String(r.to).padStart(3)+' | '+String(r.n).padStart(6)+' | '+r.tpd.padStart(5)+' | '+r.wr.toFixed(1).padStart(5)+'% | '+r.pf.toFixed(2).padStart(6)+' | '+r.pnl.toFixed(0).padStart(8)+' | '+r.npm+'/'+r.nmo+'  | '+r.npp+'/8'+tag);
  }
