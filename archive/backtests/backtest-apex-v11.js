#!/usr/bin/env node
'use strict';
// APEX V11 — Daily Loss Limit + Focused Grid around V6 best + ADX Filter + Multi-Position + Massive Grid Search
// V5 showed: XRP PF=1.45, DOGE PF=1.30, Mar/Apr profitable. Trending regimes have edge.
// V6: per-pair feature selection, ADX>25 filter, up to 5 concurrent positions, 200 configs
...
function compF(d,fr){
  ...
  F('ADXv',i=>(adx2.adx[i]-25)/25);F('ADXs',i=>i>0?(adx2.adx[i]-adx2.adx[i-1])/10:0);
  ...
  F('FR',i=>-gfr(t[i])*1000);F('FRz',i=>-gfrz(t[i]));
  F('FRc3',i=>-gfrc(t[i],3)*1000);F('FRc6',i=>-gfrc(t[i],6)*1000);
...
function engine(sigs,parsed,pr,cfg){
  const{slM,tpR,trail,maxPos,timeout}=cfg;
  let cap=INIT_CAP,pk=INIT_CAP,mdd=0;const trades=[];const positions=[];const dayPnl={};
  const sorted=sigs.slice().sort((a,b)=>a.ts-b.ts);
  function cls(p2,pd,j,tp2){let ep2;if(tp2==='SL')ep2=p2.dir===1?p2.slP*(1-SLIP_SL):p2.slP*(1+SLIP_SL);else if(tp2==='TP')ep2=p2.tpP;else ep2=pd.c[j];const g=p2.dir===1?(ep2-p2.ep)*p2.qty:(p2.ep-ep2)*p2.qty,f=POS_SIZE*FEE_E+POS_SIZE*(tp2==='TP'?FEE_TP:FEE_SL);cap+=g-f;pk=Math.max(pk,cap);const dd=pk>0?(pk-cap)/pk:0;if(dd>mdd)mdd=dd;const _net2=g-f;const _day2=new Date(pd.t[j]).toISOString().slice(0,10);if(!dayPnl)dayPnl={};if(!dayPnl[_day2])dayPnl[_day2]=0;dayPnl[_day2]+=_net2;trades.push({dir:p2.dir,pnl:_net2,type:tp2,pair:p2.pair,bars:j-p2.eb,date:new Date(pd.t[j]).toISOString().slice(0,10)});}
  function advAll(maxT){for(let p=positions.length-1;p>=0;p--){const p2=positions[p],pd=parsed[p2.pair];let closed=false;for(let j=p2.nc;j<pd.n&&j<=p2.exp&&pd.t[j]<=maxT;j++){if(trail){const mfe=p2.dir===1?(pd.h[j]-p2.ep)/p2.ep:(p2.ep-pd.l[j])/p2.ep;if(mfe>=p2.slD&&!p2.trd){p2.slP=p2.dir===1?p2.ep*1.0005:p2.ep*0.9995;p2.trd=true;}if(p2.trd&&mfe>=p2.slD*2){const ns=p2.dir===1?p2.ep*(1+p2.slD*0.7):p2.ep*(1-p2.slD*0.7);if(p2.dir===1&&ns>p2.slP)p2.slP=ns;if(p2.dir===-1&&ns<p2.slP)p2.slP=ns;}}let hS,hT;if(p2.dir===1){hS=pd.l[j]<=p2.slP;hT=pd.h[j]>=p2.tpP;}else{hS=pd.h[j]>=p2.slP;hT=pd.l[j]<=p2.tpP;}if(hS&&hT)hT=false;if(hS){cls(p2,pd,j,'SL');positions.splice(p,1);closed=true;break;}if(hT){cls(p2,pd,j,'TP');positions.splice(p,1);closed=true;break;}p2.nc=j+1;}if(!closed&&p2.nc>p2.exp){cls(p2,parsed[p2.pair],Math.min(p2.exp,parsed[p2.pair].n-1),'TO');positions.splice(p,1);}}}
  for(const sig of sorted){if(cap<=0)break;const d=parsed[sig.pair];const eb=sig.bar+1;if(eb>=d.n)continue;advAll(d.t[eb]);if(positions.length>=maxPos)continue;const sigDay=new Date(d.t[eb]).toISOString().slice(0,10);if(!dayPnl)dayPnl={};if(!dayPnl[sigDay])dayPnl[sigDay]=0;if(dayPnl[sigDay]<=-30)continue;if(positions.some(p=>p.pair===sig.pair))continue;if(pr()>=0.75)continue;if(cap<50)continue;const ep=d.o[eb],atr2=at(d.h,d.l,d.c),ap=atr2[sig.bar]/d.c[sig.bar];if(ap<=0||isNaN(ap))continue;let sl=Math.max(0.003,Math.min(0.03,ap*slM)),tp=Math.max(0.005,Math.min(0.08,sl*tpR));positions.push({pair:sig.pair,dir:sig.dir,ep,slP:sig.dir===1?ep*(1-sl):ep*(1+sl),tpP:sig.dir===1?ep*(1+tp):ep*(1-tp),qty:POS_SIZE/ep,eb,exp:eb+timeout,nc:eb+1,slD:sl,trd:false});}
...
async function main(){
  const B='═'.repeat(72);
  console.log(`\n${B}\n  APEX V7 — FOCUSED Grid around V6 best + ADX Filter + Multi-Pos + Massive Grid\n${B}`);
  const startTs=END_TS-DAYS*864e5;const allData={};
  for(const pair of PAIRS){process.stdout.write(`  ${pair}: `);const kl=await gK(pair,'1h',startTs,END_TS);process.stdout.write(`1h=${kl.length} `);const fr=await gF(pair,startTs,END_TS);console.log(`fr=${fr.length}`);allData[pair]={kl,fr};}

  // Generate massive config grid
  const slMs=[0.7,0.8,0.9,1.0,1.1,1.2];
  const tpRs=[2.0,2.3,2.5,2.7,3.0,3.5];
  const fwds=[3,4,5,6];
  const thrPs=[58,60,63,65,67,70];
  const trails=[false];
  const maxPoss=[2,3];
  const timeouts=[30,36,42,48];
  const adxFilters=[22,25,28]; // 0=no filter

  let bestPF=0,bestCfg=null,bestTrades=[],tested=0;
  const nW=Math.floor((DAYS-TRAIN_D-TEST_D)/STEP_D)+1;

  console.log(`\n  Grid: ${slMs.length}×${tpRs.length}×${fwds.length}×${thrPs.length}×${trails.length}×${maxPoss.length}×${timeouts.length}×${adxFilters.length} = massive search`);
  console.log('  Sampling ~500 +DailyLossLimit configs from grid...\n');

  const rng=P(42);
  const pick=a=>a[Math.floor(rng()*a.length)];

  for(let attempt=0;attempt<500;attempt++){
    const cfg={slM:pick(slMs),tpR:pick(tpRs),fwd:pick(fwds),thrP:pick(thrPs),
      trail:pick(trails),maxPos:pick(maxPoss),timeout:pick(timeouts),adxF:pick(adxFilters),
      mc:0.006+rng()*0.008}; // random minCorr 0.008-0.020
    ...
    for(let w=0;w<nW&&!skip;w++){
      ...
      for(const pair of PAIRS){
        ...
        const sel=co.map((c,i)=>({idx:i,corr:c,abs:Math.abs(c)})).filter(x=>x.abs>=cfg.mc).sort((a,b)=>b.abs-a.abs).slice(0,10);
        if(sel.length<2)continue;
        // Threshold
        let tc=[];for(let i=55;i<d.n;i++){
          if(cfg.adxF>0&&adxArr[i]<cfg.adxF)continue;
          let comp=0;for(const{idx,corr}of sel)comp+=corr*fs[idx][i];tc.push(Math.abs(comp));}
        tc.sort((a,b)=>a-b);const thr=tc[Math.floor(tc.length*cfg.thrP/100)]||0.001;
        pairModels[pair]={sel,thr};
      }
      ...
      const tSigs=[],tPar={};
      for(const pair of PAIRS){
        if(!pairModels[pair])continue;
        const tkl=allData[pair].kl.filter(k=>parseInt(k[0])>=tes&&parseInt(k[0])<tee);
        if(tkl.length<50)continue;
        const d=pa(tkl);tPar[pair]=d;
        const{fs,adx:adxArr}=compF(d,allData[pair].fr);
        const{sel,thr}=pairModels[pair];
        let last=-2;
        for(let i=55;i<d.n-cfg.timeout-1;i++){
    ...
    tested++;
    const s=st(allOOS);
    if(s.pf>bestPF&&s.n>=50){
      bestPF=s.pf;bestCfg=cfg;bestTrades=allOOS;
      const tpd=s.n/(nW*TEST_D);
      console.log(`  [${tested}] sl=${cfg.slM} tp=${cfg.tpR} fwd=${cfg.fwd} thr=${cfg.thrP} tr=${cfg.trail?'Y':'N'} mp=${cfg.maxPos} to=${cfg.timeout} adx=${cfg.adxF} mc=${cfg.mc.toFixed(3)} | ${s.n}t WR ${s.wr.toFixed(1)}% PF ${s.pf.toFixed(2)} $${s.pnl.toFixed(0)} ~${tpd.toFixed(1)}t/d ◄`);
    }
  }

  if(!bestCfg){console.log('\n  No viable config.');return;}
  const s=st(bestTrades);
  console.log(`\n${B}\n  BEST: sl=${bestCfg.slM} tp=${bestCfg.tpR} fwd=${bestCfg.fwd} thr=${bestCfg.thrP} trail=${bestCfg.trail} mp=${bestCfg.maxPos} to=${bestCfg.timeout} adx=${bestCfg.adxF}\n${B}`);
  console.log(`  PF: ${s.pf.toFixed(2)} | WR: ${s.wr.toFixed(1)}% | $${s.pnl.toFixed(0)} | ${s.n} trades (TP:${s.tp} SL:${s.sl} TO:${s.to})`);

  const mos={};for(const t of bestTrades){const m=t.date?.slice(0,7)||'?';if(!mos[m])mos[m]=[];mos[m].push(t);}
  let pm=0;console.log('\n  Monthly:');
  for(const m of Object.keys(mos).sort()){const s2=st(mos[m]);if(s2.pnl>0)pm++;console.log(`  ${m}: ${s2.n}t WR ${s2.wr.toFixed(1)}% PF ${s2.pf.toFixed(2)} $${s2.pnl.toFixed(0)}`);}

  const pairs={};for(const t of bestTrades){if(!pairs[t.pair])pairs[t.pair]=[];pairs[t.pair].push(t);}
  let pp=0;console.log('\n  Per Pair:');
  for(const p of Object.keys(pairs).sort()){const s2=st(pairs[p]);if(s2.pnl>0)pp++;console.log(`  ${p.padEnd(10)}: ${s2.n}t WR ${s2.wr.toFixed(1)}% PF ${s2.pf.toFixed(2)} $${s2.pnl.toFixed(0)}`);}
