#!/usr/bin/env node
'use strict';
// Mejora 1: Kill Switch por par — sensitivity analysis 3×3×3 = 27 combos
// Takes V40 signal generation, applies per-pair rolling PF + cooldown logic
// Measures: PF, WR, t/d, DD, Sharpe per combination
...
async function genAllSignals(allData){
  ...
  for(let w=0;w<nW;w++){
    ...
    for(const pair of PAIRS){
      ...
      const Fm=momFeatures(tB);const Fr=revFeatures(tB);
      const fwd=new Float64Array(Fm.n).fill(NaN);for(let i=50;i<Fm.n-2;i++)fwd[i]=(Fm.c[i+2]-Fm.c[i])/Fm.c[i]*100;
      const coM=pearson(Fm.fs,fwd,50,Fm.n-2);
      const coR=pearson(Fr.fs,fwd,50,Fr.n-2);
      const selM=coM.map((c,i)=>({idx:i,corr:c,abs:Math.abs(c)})).filter(x=>x.abs>=0.011).sort((a,b)=>b.abs-a.abs).slice(0,6);
      const selR=coR.map((c,i)=>({idx:i,corr:c,abs:Math.abs(c)})).filter(x=>x.abs>=0.011).sort((a,b)=>b.abs-a.abs).slice(0,6);
      if(selM.length<2)continue;
      let tcM=[];for(let i=55;i<Fm.n;i++){if(Fm.adx[i]<22)continue;let comp=0;for(const{idx,corr}of selM)comp+=corr*Fm.fs[idx][i];tcM.push(Math.abs(comp));}tcM.sort((a,b)=>a-b);
      let tcR=[];for(let i=55;i<Fr.n;i++){if(Fr.adx[i]<22)continue;let comp=0;for(const{idx,corr}of selR)comp+=corr*Fr.fs[idx][i];tcR.push(Math.abs(comp));}tcR.sort((a,b)=>a-b);
      const thrM=tcM[Math.floor(tcM.length*0.55)]||0.001;
      const thrR=tcR[Math.floor(tcR.length*0.55)]||0.001;
    ...
    for(const pair of PAIRS){
      ...
      const Fm=momFeatures(teB);const Fr=revFeatures(teB);
      if(!parsed[pair])parsed[pair]={t:[...Fm.t],o:[...Fm.o],h:[...Fm.h],l:[...Fm.l],c:[...Fm.c],atr:[...Fm.atr]};
      else{for(let i=0;i<Fm.t.length;i++){parsed[pair].t.push(Fm.t[i]);parsed[pair].o.push(Fm.o[i]);parsed[pair].h.push(Fm.h[i]);parsed[pair].l.push(Fm.l[i]);parsed[pair].c.push(Fm.c[i]);parsed[pair].atr.push(Fm.atr[i]);}}
      const{selM,selR,thrM,thrR}=pm[pair];
      const trend4=compute4HTrend(te4);const t4=te4.map(b=>b.t);
      let last=-3;
      for(let i=55;i<Fm.n-60-1;i++){
        if(i-last<2)continue;if(Fm.adx[i]<22)continue;
        ...
        const passM=Math.abs(compM)>=thrM;const passR=Math.abs(compR)>=thrR;
        const dirM=compM>0?1:-1;const dirR=-1*(compR>0?1:-1);
        let finalDir=0;
        if(passM&&passR&&dirM===dirR)finalDir=dirM;else if(passM)finalDir=dirM;else if(passR)finalDir=dirR;
        if(finalDir===0)continue;
        const b4=findPrev(t4,Fm.t[i]);if(b4<0||trend4[b4]!==finalDir)continue;
        // Get absolute index in merged parsed array
        const absIdx=parsed[pair].t.length-Fm.n+i;
        signals.push({pair,ts:Fm.t[i],dir:finalDir,absIdx,atr:Fm.atr[i],ep:Fm.c[i]});
        last=i;
      }
    }
  }
...
// Engine with kill switch logic
function engineKillSwitch(signals,parsed,cfg){
  let cap=INIT_CAP;const trades=[];
  const slots=new Array(4).fill(null);  // V40 maxPos=4
  const pairState={};  // pair → {status:'active'|'cooldown', cooldownEnd, recentTrades: [], cooldownsCount}
  for(const p of PAIRS)pairState[p]={status:'active',cooldownEnd:0,recentTrades:[],cooldownsCount:0,shadowTrades:[]};
  const prng=P(SEED);

  function cls(si,j,reason){const pos=slots[si];const pd=parsed[pos.pair];let ep2;if(reason==='SL')ep2=pos.dir===1?pos.slP*(1-SLIP_SL):pos.slP*(1+SLIP_SL);else if(reason==='TP')ep2=pos.tpP;else ep2=pd.c[j];const g=pos.dir===1?(ep2-pos.ep)*pos.qty:(pos.ep-ep2)*pos.qty;const f=POS_SIZE*FEE_E+POS_SIZE*(reason==='TP'?FEE_TP:FEE_SL);const pnl=g-f;cap+=pnl;const ts=pd.t[j];const tr={dir:pos.dir,pnl,type:reason,pair:pos.pair,bars:j-pos.eb,date:new Date(ts).toISOString().slice(0,10),ts,isShadow:pos.isShadow||false};if(pos.isShadow){pairState[pos.pair].shadowTrades.push(tr);}else{trades.push(tr);pairState[pos.pair].recentTrades.push(tr);}slots[si]=null;}
  function advance(upTs){for(let si=0;si<4;si++){const pos=slots[si];if(!pos)continue;const pd=parsed[pos.pair];for(let j=pos.nc;j<pd.c.length&&j<=pos.exp&&pd.t[j]<=upTs;j++){let hS,hT;if(pos.dir===1){hS=pd.l[j]<=pos.slP;hT=pd.h[j]>=pos.tpP;}else{hS=pd.h[j]>=pos.slP;hT=pd.l[j]<=pos.tpP;}if(hS&&hT)hT=false;if(hS){cls(si,j,'SL');break;}if(hT){cls(si,j,'TP');break;}pos.nc=j+1;}if(slots[si]&&slots[si].nc>slots[si].exp){cls(si,Math.min(slots[si].exp,pd.c.length-1),'TO');}}}
  
  function computeRollingPF(trs,atTs,windowDays){
    const cutoff=atTs-windowDays*86400000;
    const recent=trs.filter(t=>t.ts>=cutoff);
    if(recent.length<5)return null;
    const w=recent.filter(t=>t.pnl>0).reduce((s,t)=>s+t.pnl,0);
    const l=Math.abs(recent.filter(t=>t.pnl<=0).reduce((s,t)=>s+t.pnl,0));
    return l>0?w/l:(w>0?99:0);
  }

  function updatePairState(ts){
    for(const p of PAIRS){
      const ps=pairState[p];
      if(ps.status==='active'){
        const rpf=computeRollingPF(ps.recentTrades,ts,cfg.rollingDays);
        if(rpf!==null&&rpf<cfg.thrEnter){
          ps.status='cooldown';
          ps.cooldownEnd=ts+cfg.cooldownDays*86400000;
          ps.cooldownsCount++;
          ps.shadowTrades=[];
        }
      }else if(ps.status==='cooldown'){
        if(ts>=ps.cooldownEnd){
          // Check shadow PF
          const spf=computeRollingPF(ps.shadowTrades,ts,cfg.cooldownDays);
          if(spf!==null&&spf>=cfg.thrExit){
            ps.status='active';
            ps.shadowTrades=[];
          }else if(ps.cooldownsCount>=3){
            ps.status='dead';
          }else{
            // Extend cooldown
            ps.cooldownEnd=ts+cfg.cooldownDays*86400000;
  ...
  for(const sig of signals){
    if(cap<=0)break;
    updatePairState(sig.ts);
    const ps=pairState[sig.pair];
    if(ps.status==='dead')continue;
    const pd=parsed[sig.pair];
    const absIdx=sig.absIdx;
    const eb=absIdx+1;if(eb>=pd.c.length)continue;
    advance(pd.t[eb]);
    // If in cooldown: shadow trade (doesn't affect cap)
    const isShadow=(ps.status==='cooldown');
    // Position constraints (slot + same pair)
    let freeSlot=-1;
    if(!isShadow){
      let conflict=false;for(let sj=0;sj<4;sj++)if(slots[sj]&&slots[sj].pair===sig.pair&&!slots[sj].isShadow){conflict=true;break;}
      if(conflict)continue;
      for(let si=0;si<4;si++){if(!slots[si]||(slots[si]&&slots[si].isShadow)){freeSlot=si;break;}}
      if(freeSlot===-1)continue;
      if(prng()>=0.75)continue;
      if(cap<50)continue;
    }
    const ep=pd.o[eb],atrA=pd.atr[absIdx],ap=atrA/pd.c[absIdx];if(ap<=0||isNaN(ap))continue;
    const slPct=Math.max(0.003,Math.min(0.03,ap*2));
    const tpPct=Math.max(0.005,Math.min(0.08,slPct*1.625));
    const pos={pair:sig.pair,dir:sig.dir,ep,slP:sig.dir===1?ep*(1-slPct):ep*(1+slPct),tpP:sig.dir===1?ep*(1+tpPct):ep*(1-tpPct),qty:POS_SIZE/ep,eb,exp:eb+60,nc:eb+1,isShadow};
    if(isShadow){
      // Simulate shadow trade inline (doesn't block slots)
      let result=null,exitP=ep,exitIdx=eb,closedAtReason='TO';
      for(let j=eb+1;j<=Math.min(eb+60,pd.c.length-1);j++){
        let hS,hT;if(pos.dir===1){hS=pd.l[j]<=pos.slP;hT=pd.h[j]>=pos.tpP;}else{hS=pd.h[j]>=pos.slP;hT=pd.l[j]<=pos.tpP;}
        if(hS&&hT)hT=false;
        if(hS){result='L';exitP=sig.dir===1?pos.slP*(1-SLIP_SL):pos.slP*(1+SLIP_SL);exitIdx=j;closedAtReason='SL';break;}
        if(hT){result='W';exitP=pos.tpP;exitIdx=j;closedAtReason='TP';break;}
      }
      if(!result){exitP=pd.c[Math.min(eb+60,pd.c.length-1)];exitIdx=Math.min(eb+60,pd.c.length-1);}
      const g=pos.dir===1?(exitP-pos.ep)*pos.qty:(pos.ep-exitP)*pos.qty;
      const f=POS_SIZE*FEE_E+POS_SIZE*(closedAtReason==='TP'?FEE_TP:FEE_SL);
      ps.shadowTrades.push({ts:pd.t[exitIdx],pnl:g-f,type:closedAtReason});
    }else{
      slots[freeSlot]=pos;
    }
  }
  advance(Infinity);for(let si=0;si<4;si++){if(slots[si]){const pd=parsed[slots[si].pair];cls(si,Math.min(slots[si].exp,pd.c.length-1),'TO');}}
  return{trades,pairState};
}

async function main(){
  console.log('Mejora 1: Kill Switch — Sensitivity Analysis 3×3×3');
  console.log('─'.repeat(80));
  console.log('Loading data + generating V40 signals...');
  const allData={};
  for(const pair of PAIRS){
  ...
  console.log(`Generated ${signals.length} signals across all walk-forward windows\n`);

  // Baseline: V40 (no kill switch)
  const baselineCfg={rollingDays:9999,cooldownDays:9999,thrEnter:-1,thrExit:-1};
  const baselineRes=engineKillSwitch(signals,parsed,baselineCfg);
  const baseS=st(baselineRes.trades);
  console.log(`BASELINE V40: ${baseS.n}t PF${baseS.pf.toFixed(2)} WR${baseS.wr.toFixed(1)}% DD$${baseS.mdd.toFixed(0)} Sh${baseS.sharpe.toFixed(1)} $${baseS.pnl.toFixed(0)}`);
  const baseDD_pct=(baseS.mdd/(INIT_CAP+baseS.pnl))*100;

  // 3×3×3 = 27 combinations
  const thresholds=[0.95,1.00,1.05];
  const windows=[20,30,45];
  const durations=[10,15,20];
  const results=[];
  console.log('\nSensitivity analysis:');
  console.log('  Threshold × Rolling × Cooldown → PF / WR / t/d / DD% / Sharpe / PnL');
  for(const thr of thresholds){
    for(const win of windows){
      for(const dur of durations){
        const cfg={thrEnter:thr,thrExit:1.10,rollingDays:win,cooldownDays:dur};
        const r=engineKillSwitch(signals,parsed,cfg);
        const s=st(r.trades);
        const deadPairs=Object.values(r.pairState).filter(p=>p.status==='dead').length;
        const cooldownPairs=Object.values(r.pairState).filter(p=>p.status==='cooldown').length;
        const tpd=s.n/150;
        const ddPct=(s.mdd/(INIT_CAP+s.pnl))*100;
        results.push({cfg,s,tpd,deadPairs,cooldownPairs,ddPct});
        console.log(`  P<${thr.toFixed(2)} W${win}d C${dur}d → PF${s.pf.toFixed(2)} WR${s.wr.toFixed(1)}% ${tpd.toFixed(1)}t/d DD${ddPct.toFixed(0)}% Sh${s.sharpe.toFixed(1)} $${s.pnl.toFixed(0)} (${deadPairs}dead ${cooldownPairs}cd)`);
      }
    }
  }

  // Sort by Sharpe (user's criterion)
  const bySharpe=[...results].sort((a,b)=>b.s.sharpe-a.s.sharpe);
  const byPF=[...results].sort((a,b)=>b.s.pf-a.s.pf);
  console.log('\n── TOP 5 BY SHARPE ──');
  for(const r of bySharpe.slice(0,5))console.log(`  thr<${r.cfg.thrEnter} win${r.cfg.rollingDays}d cd${r.cfg.cooldownDays}d → Sh${r.s.sharpe.toFixed(1)} PF${r.s.pf.toFixed(2)} DD${r.ddPct.toFixed(0)}%`);
  console.log('\n── TOP 5 BY PF ──');
  for(const r of byPF.slice(0,5))console.log(`  thr<${r.cfg.thrEnter} win${r.cfg.rollingDays}d cd${r.cfg.cooldownDays}d → PF${r.s.pf.toFixed(2)} Sh${r.s.sharpe.toFixed(1)} DD${r.ddPct.toFixed(0)}%`);

  console.log('\n── DELTA vs BASELINE ──');
  const best=bySharpe[0];
  console.log(`Baseline: PF${baseS.pf.toFixed(2)} WR${baseS.wr.toFixed(1)}% DD${baseDD_pct.toFixed(0)}% Sh${baseS.sharpe.toFixed(1)} $${baseS.pnl.toFixed(0)}`);
  console.log(`Best:     PF${best.s.pf.toFixed(2)} WR${best.s.wr.toFixed(1)}% DD${best.ddPct.toFixed(0)}% Sh${best.s.sharpe.toFixed(1)} $${best.s.pnl.toFixed(0)}`);
  console.log(`Delta:    PF ${(best.s.pf-baseS.pf).toFixed(2)} WR ${(best.s.wr-baseS.wr).toFixed(1)}% DD ${(best.ddPct-baseDD_pct).toFixed(0)}% Sh ${(best.s.sharpe-baseS.sharpe).toFixed(1)} $${(best.s.pnl-baseS.pnl).toFixed(0)}`);
  const acceptable=(best.s.pf-baseS.pf)>=0.08 && (baseDD_pct-best.ddPct)>=10;
  console.log(`\nUser criterion (PF ≥+0.08 AND DD ≥-10%): ${acceptable?'★ MET — integrar':'✗ NOT MET — investigar'}`);
}
