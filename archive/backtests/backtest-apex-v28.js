#!/usr/bin/env node
'use strict';
// APEX V28 — HEAD-TO-HEAD: V16's exact config at maxPos=1 vs maxPos=2 vs maxPos=3
// Pin every other parameter to V16's known-best values. Only vary maxPos.
...
// Engine supports configurable maxPos with pair-diversification
...
// Run ONE config end-to-end — returns full stats
function runConfig(cfg, allData){
  ...
  for(let w=0;w<nW;w++){
    ...
    if(Object.keys(pairModels).length<4)continue;
    const tSigs=[],tPar={};
  ...
  let pm=0;for(const m of Object.keys(mos))if(st(mos[m]).pnl>0)pm++;
  return {s, tpd, pp, pm, pairs, mos, allOOS};
}

async function main(){
  const B='═'.repeat(75);
  console.log(`\n${B}\n APEX V28 — HEAD-TO-HEAD: V16 exact config at maxPos=1, 2, 3\n${B}\n`);
  ...
  // V16 KNOWN BEST config (verified PF 1.87)
  const BASE_CFG = {slM:2.5, tpR:3.0, fwd:2, thrP:65, timeout:60, adxF:25, mc:0.005};

  const results=[];

  // Test maxPos ∈ {1, 2, 3}
  for(const maxPos of [1, 2, 3]){
    const cfg = {...BASE_CFG, maxPos};
    console.log(`\n▶ Testing maxPos=${maxPos}`);
    const r = runConfig(cfg, allData);
    console.log(`  PF ${r.s.pf.toFixed(2)} | WR ${r.s.wr.toFixed(1)}% | ${r.s.n}t | ${r.tpd.toFixed(2)}/d | pp=${r.pp}/8 | pm=${r.pm}/${Object.keys(r.mos).length} | $${r.s.pnl.toFixed(0)}`);
    results.push({maxPos, ...r});
  }

  // Also test a COMBINATION: V16 base + maxPos=2 + lower threshold (more signals)
  // to see if we can hit 2 t/d with higher PF
  console.log('\n═══ EXPLORING thrP variations at maxPos=2 ═══');
  for(const thrP of [45, 50, 55, 60, 65, 70]){
    const cfg = {...BASE_CFG, maxPos:2, thrP};
    const r = runConfig(cfg, allData);
    console.log(`  thrP=${thrP}: PF ${r.s.pf.toFixed(2)} | ${r.s.n}t | ${r.tpd.toFixed(2)}/d | pp=${r.pp}/8 | $${r.s.pnl.toFixed(0)}`);
    results.push({label:`mp2-thr${thrP}`, maxPos:2, thrP, ...r});
  }

  // Also try different slM/tpR combos at maxPos=2
  console.log('\n═══ EXPLORING slM/tpR at maxPos=2, thrP=60 ═══');
  for(const slM of [2.0, 2.5, 3.0]){
    for(const tpR of [2.5, 3.0, 3.5]){
      const cfg = {...BASE_CFG, maxPos:2, thrP:60, slM, tpR};
      const r = runConfig(cfg, allData);
      console.log(`  slM=${slM} tpR=${tpR}: PF ${r.s.pf.toFixed(2)} | ${r.s.n}t | ${r.tpd.toFixed(2)}/d | pp=${r.pp}/8 | $${r.s.pnl.toFixed(0)}`);
      results.push({label:`mp2-sl${slM}-tp${tpR}`, ...r, cfg});
    }
  }

  // Sort and show best
  results.sort((a,b) => {
    // Prefer configs that hit >= 2 t/d, then by PF
    const aHit = a.tpd>=2 ? 1 : 0, bHit = b.tpd>=2 ? 1 : 0;
    if(aHit !== bHit) return bHit - aHit;
    return b.s.pf - a.s.pf;
  });

  console.log(`\n${B}\n BEST CANDIDATES (prioritizing tpd>=2):\n${B}`);
  for(let i=0; i<Math.min(10, results.length); i++){
    const r = results[i];
    const tag = r.label || `maxPos=${r.maxPos}`;
    const hit2 = r.tpd>=2 ? '✓' : '✗';
    console.log(`  #${i+1} [${tag}] PF ${r.s.pf.toFixed(2)} | WR ${r.s.wr.toFixed(1)}% | ${r.s.n}t | ${r.tpd.toFixed(2)}/d ${hit2} | pp=${r.pp}/8 | $${r.s.pnl.toFixed(0)}`);
  }
}
