...
const VIP=['BTCUSDT','ETHUSDT','SOLUSDT','BNBUSDT','XRPUSDT','AVAXUSDT','ADAUSDT','DOTUSDT','LINKUSDT','APTUSDT','NEARUSDT','FILUSDT','UNIUSDT','ARBUSDT','OPUSDT','SHIBUSDT','SUIUSDT','SEIUSDT','TIAUSDT','JUPUSDT','WIFUSDT','FETUSDT','PEPEUSDT'];
const SCA=['BTCUSDT','ETHUSDT','SOLUSDT','BNBUSDT','XRPUSDT','DOGEUSDT','AVAXUSDT','LINKUSDT','ADAUSDT','DOTUSDT','NEARUSDT','APTUSDT','ARBUSDT','OPUSDT','SUIUSDT','SEIUSDT','FILUSDT','UNIUSDT','JUPUSDT','WIFUSDT','FETUSDT','PEPEUSDT','TIAUSDT','ATOMUSDT'];
const FRE=['BTCUSDT','ETHUSDT','SOLUSDT','BNBUSDT'];
...
function calcKC(H,L,C){if(C.length<21)return{position:0.5};const mid=calcEMA(C,20);const at=calcATR(H,L,C,14);const u=mid+2*at,lo=mid-2*at;const r=u-lo;return{position:r>0?(C.at(-1)-lo)/r:0.5};}

function fetchK(sym,itv,lim,st){return new Promise((res,rej)=>{let u=`https://api.binance.com/api/v3/klines?symbol=${sym}&interval=${itv}&limit=${lim}`;if(st)u+=`&startTime=${st}`;https.get(u,r=>{let d='';r.on('data',c=>d+=c);r.on('end',()=>{try{const p=JSON.parse(d);if(p.code)rej(new Error(p.msg));else res(p);}catch(e){rej(e);}});}).on('error',rej);});}
async function fetchAll(sym,itv,total){const a=[];const ms={'5m':3e5,'15m':9e5,'1h':36e5}[itv]||3e5;let st=Date.now()-total*ms;while(a.length<total){const l=Math.min(total-a.length,1000);try{const d=await fetchK(sym,itv,l,st);if(!Array.isArray(d)||!d.length)break;a.push(...d);st=d[d.length-1][0]+ms;}catch(e){break;}await new Promise(r=>setTimeout(r,120));}return a;}
...
function genSig(C5,H5,L5,V5,C15,H15,L15,C1h,H1h,L1h,T5,mode,sym){
  ...
  const dow=new Date(T5.at(-1)).getUTCDay();
  ...
  if(isStrict&&!isTrending){
    ...
    const l4=C.slice(-4);const bExh=l4.length>=4&&l4.every((x,i)=>i===0||x<l4[i-1]);const buExh=l4.length>=4&&l4.every((x,i)=>i===0||x>l4[i-1]);
  ...
  if(isStrict){
    ...
    // F2: Block toxic hours only (7 blocked, 17 allowed)
    if(sig!=='NEUTRAL'&&![6,7,13,14,19,20,21,22,23].includes(hU))sig='NEUTRAL';
    // F2b: Marginal hours need higher conviction
    if(sig!=='NEUTRAL'&&false){const cs=sig==='BUY'?B:S;if(cs<10)sig='NEUTRAL';}
    // F3: volatile
    if(sig!=='NEUTRAL'&&isVolatile)sig='NEUTRAL';
    // F4: Saturday
    if(sig!=='NEUTRAL'&&dow===6)sig='NEUTRAL';
    // Min vol
    ...
    // Block toxic hours (5 blocked, 19 allowed)
    if(sig!=='NEUTRAL'&&[1,2,3,4,8].includes(hU))sig='NEUTRAL';
    // Marginal hours need higher conviction
    if(sig!=='NEUTRAL'&&false)sig='NEUTRAL';
    // Block Tuesday
    if(sig!=='NEUTRAL'&&dow===2)sig='NEUTRAL';
    // Dominance filter
    if(sig!=='NEUTRAL'&&Math.max(B,S)<Math.min(B,S)*1.3)sig='NEUTRAL';
    // Dead volume
    ...
    if(B>S&&B>=5&&bI>=2)sig='BUY';else if(S>B&&S>=5&&sI>=2)sig='SELL';
    if(sig!=='NEUTRAL'&&isTrending)sig='NEUTRAL';if(sig!=='NEUTRAL'&&isVolatile)sig='NEUTRAL';
  ...
  const cb=cur*0.0008;let tp,sl;
  if(sig==='BUY'){tp=cur+tpD+cb;sl=cur-slD-cb;}else if(sig==='SELL'){tp=cur-tpD-cb;sl=cur+slD+cb;}
  return{signal:sig,conf,entry:cur,tp,sl,tpD,slD,B,S,bI,sI,regime:reg,hU,adx:adx.adx,rsi,atr:bATR,dow};
}

function simTrade(sg,fc,to){const e=sg.entry,tp=sg.tp,sl=sg.sl,iB=sg.signal==='BUY';for(let i=0;i<Math.min(fc.length,to);i++){const c=fc[i];if(iB){if(c.l<=sl)return{r:'SL',ep:sl,b:i+1};if(c.h>=tp)return{r:'TP',ep:tp,b:i+1};}else{if(c.h>=sl)return{r:'SL',ep:sl,b:i+1};if(c.l<=tp)return{r:'TP',ep:tp,b:i+1};}}const li=Math.min(fc.length-1,to-1);if(li<0)return null;return{r:'TO',ep:fc[li].c,b:li+1};}

async function runBT(mode,syms,to=50){
  console.log(`\n${'═'.repeat(60)}\n  ${mode.toUpperCase()} | ${syms.length} pairs | timeout=${to}\n${'═'.repeat(60)}`);
  const cd=mode==='strict'?24:mode==='scalp'?8:8;const data={};
  ...
  const fc=k5.slice(i+1,i+1+to).map(k=>({h:k.h,l:k.l,c:k.c}));const r=simTrade(sg,fc,to);if(!r)continue;
  let pnl=sg.signal==='BUY'?(r.ep-sg.entry)/sg.entry*100:(sg.entry-r.ep)/sg.entry*100;pnl-=0.08;
  trades.push({sym,sig:sg.signal,res:r.r,pnl,bars:r.b,dur:r.b*5,h:sg.hU,reg:sg.regime,dow:sg.dow});
  ...
  if(!dwS[sg.dow])dwS[sg.dow]={t:0,w:0,p:0};dwS[sg.dow].t++;dwS[sg.dow].p+=pnl;if(r.r==='TP')dwS[sg.dow].w++;}}
  const n=trades.length;if(n===0){console.log('NO TRADES');return;}
  const wins=trades.filter(t=>t.res==='TP').length;const wr=wins/n*100;const tP=trades.reduce((s,t)=>s+t.pnl,0);
  const gp=trades.filter(t=>t.pnl>0).reduce((s,t)=>s+t.pnl,0);const gl=Math.abs(trades.filter(t=>t.pnl<0).reduce((s,t)=>s+t.pnl,0));const pf=gl>0?gp/gl:99;
  let eq=10000,pk=10000,mdd=0;for(const t of trades){eq+=500*5*(t.pnl/100);if(eq>pk)pk=eq;const dd=(pk-eq)/pk*100;if(dd>mdd)mdd=dd;}
  const aW=wins>0?gp/wins:0;const lc=n-wins;const aL=lc>0?gl/lc:0;const exp=(wr/100)*aW-(1-wr/100)*aL;
  ...
  console.log(`  │ PF: ${pf.toFixed(3)} | DD: ${mdd.toFixed(2)}% | AvgW: +${aW.toFixed(3)}% AvgL: -${aL.toFixed(3)}%`);
  console.log(`  │ Expect: ${exp>=0?'+':''}${exp.toFixed(4)} | Allowed hours: ${mode==='strict'?'17/24':mode==='scalp'?'19/24':'6/24'}`);
  ...
  console.log(`\n  HOURS:`);for(let h=0;h<24;h++){const d=hS[h];if(!d)continue;console.log(`  H${String(h).padStart(2)} ${String(d.t).padStart(5)} WR:${(d.w/d.t*100).toFixed(0).padStart(3)}% PnL:${(d.p>=0?'+':'')+d.p.toFixed(2)}`);}
  console.log(`\n  REGIME:`);Object.entries(rS).forEach(([r,d])=>console.log(`  ${r.padEnd(10)} ${String(d.t).padStart(5)} WR:${(d.w/d.t*100).toFixed(0).padStart(3)}% PnL:${(d.p>=0?'+':'')+d.p.toFixed(2)}`));
  console.log(`\n  DOW:`);['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].forEach((dn,i)=>{const d=dwS[i];if(!d)return;console.log(`  ${dn} ${String(d.t).padStart(5)} WR:${(d.w/d.t*100).toFixed(0).padStart(3)}% PnL:${(d.p>=0?'+':'')+d.p.toFixed(2)}`);});
  console.log(`\n  EQUITY:`);let rp=0;Object.entries(dS).sort().forEach(([day,d])=>{rp+=d.p;console.log(`  ${day} ${String(d.t).padStart(3)}t WR${(d.w/d.t*100).toFixed(0).padStart(3)}% ${(d.p>=0?'+':'')+d.p.toFixed(2).padStart(7)}% cum:${(rp>=0?'+':'')+rp.toFixed(2).padStart(7)}%`);});
  return{n,wins,wr,tP,pf,mdd,eq,exp,tpd:(n/14).toFixed(0)};
}

async function main(){
  console.log('╔═══════════════════════════════════════════════╗');
  console.log('║  RXTRADING v4 FINAL — WIDE HOURS VALIDATION  ║');
  console.log('╚═══════════════════════════════════════════════╝');
  const r={};
  for(const[m,s]of[['strict',VIP],['scalp',SCA],['frequent',FRE]]){try{r[m]=await runBT(m,s,50);}catch(e){console.error(e.message);}}
  console.log(`\n${'═'.repeat(60)}\n  FINAL COMPARISON\n${'═'.repeat(60)}`);
  console.log(`  ${'Mode'.padEnd(10)} ${'T'.padStart(5)} ${'T/d'.padStart(5)} ${'WR'.padStart(6)} ${'PnL'.padStart(8)} ${'PF'.padStart(6)} ${'DD'.padStart(6)} ${'Hours'.padStart(6)}`);
  for(const m of['strict','scalp','frequent']){const x=r[m];if(!x)continue;console.log(`  ${m.padEnd(10)} ${String(x.n).padStart(5)} ${x.tpd.padStart(5)} ${x.wr.toFixed(1).padStart(6)} ${(x.tP>=0?'+':'')+x.tP.toFixed(1).padStart(7)} ${x.pf.toFixed(2).padStart(6)} ${x.mdd.toFixed(1).padStart(6)} ${m==='strict'?'17/24':m==='scalp'?'19/24':'6/24'}`);}
}
