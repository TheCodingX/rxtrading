...
async function main(){
  ...
  console.log('Sampling 2500 institutional configs...\n');
  for(let att=0;att<2500;att++){
    ...
    const qual=s.pf*Math.min(tpd/3,1)*(s.pnl>0?1:0.3);
    if(s.n>=150&&(!bestQual||qual>bestQual)){bestQual=qual;bestPF=s.pf;bestCfg=cfg;bestTr=allOOS;
