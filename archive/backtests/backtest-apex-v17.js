...
async function main(){
  const B='═'.repeat(72);
  console.log(`\n${B}\n  APEX V17 — TARGET: PF>1.84 8/8pairs 7/7months >4sig/day: Premium Index + Funding + ML\n${B}`);
  ...
  const slMs=[1.5,1.8,2.0,2.2,2.5,3.0],tpRs=[2.5,2.7,3.0,3.3,3.5,4.0],fwds=[1,2,3];
  const thrPs=[55,58,60,62,65,68,70],maxPoss=[2,3,4],timeouts=[48,60,72,84],adxFs=[0,18,20,22,25];
  let bestPF=0,bestCfg=null,bestTr=[];

  console.log('  Sampling 3000 V17-TARGET configs with institutional data...\n');
  for(let att=0;att<3000;att++){
    ...
    // Custom score: PF × (profitPairs/8) × min(1, trades/200) × (profitMonths/7)
    ...
    const _score=s.pf*(_npp/8)*Math.min(1,s.n/200)*(_npm/Math.max(Object.keys(_mo).length,1));
    if(_score>bestPF&&s.n>=50){bestPF=_score;bestCfg=cfg;bestTr=allOOS;
