#!/usr/bin/env node
'use strict';
// APEX V9 — WIDEST Grid 1200 configs around V6 best + ADX Filter + Multi-Position + Massive Grid Search
...
async function main(){
  const B='═'.repeat(72);
  console.log(`\n${B}\n  APEX V9 — WIDEST Grid 1200 configs around V6 best + ADX Filter + Multi-Pos + Massive Grid\n${B}`);
  ...
  const slMs=[0.7,0.8,0.9,1.0,1.1,1.2,1.3,1.5];
  const tpRs=[2.0,2.5,3.0,3.5,4.0,5.0];
  const fwds=[3,4,5,6,8];
  const thrPs=[50,55,58,60,63,65,70];
  const trails=[false];
  const maxPoss=[1,2,3,4];
  const timeouts=[18,24,30,36,48];
  const adxFilters=[0,18,22,25,28,30]; // 0=no filter
  ...
  console.log('  Sampling ~1200 wide+deep configs from grid...\n');
  ...
  for(let attempt=0;attempt<1200;attempt++){
    const cfg={slM:pick(slMs),tpR:pick(tpRs),fwd:pick(fwds),thrP:pick(thrPs),
      ...
      mc:0.005+rng()*0.012}; // random minCorr 0.008-0.020
