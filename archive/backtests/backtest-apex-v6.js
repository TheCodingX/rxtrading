#!/usr/bin/env node
'use strict';
// APEX V6 — Per-Pair ML + ADX Filter + Multi-Position + Massive Grid Search
...
async function main(){
  const B='═'.repeat(72);
  console.log(`\n${B}\n  APEX V6 — Per-Pair ML + ADX Filter + Multi-Pos + Massive Grid\n${B}`);
  ...
  const slMs=[0.6,0.8,1.0,1.2,1.5,2.0];
  const tpRs=[1.5,2.0,2.5,3.0,4.0];
  const fwds=[3,4,6,8];
  const thrPs=[65,70,75,80,85];
  const trails=[true,false];
  const maxPoss=[2,3,5];
  const timeouts=[12,18,24,36];
  const adxFilters=[0,20,25]; // 0=no filter
  ...
  console.log('  Sampling ~200 random configs from grid...\n');
  ...
  for(let attempt=0;attempt<200;attempt++){
    const cfg={slM:pick(slMs),tpR:pick(tpRs),fwd:pick(fwds),thrP:pick(thrPs),
      ...
      mc:0.008+rng()*0.012}; // random minCorr 0.008-0.020
