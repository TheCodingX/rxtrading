#!/usr/bin/env node
'use strict';
// APEX V26 — Grid search refinement around V25 best: slM=3.3 tpR=2.0 fwd=1 thrP=75 adxF=25 smartTO=18 fullTO=60 topK=10
// Explore tight neighborhood + aggressive variations
...
function compF(d,fr,piKl){
  ...
  F('FR',i=>-gfr(t[i])*1000);F('FRc3',i=>-gfrc(t[i],3)*1000);F('FRc6',i=>-gfrc(t[i],6)*1000);F('FRc12',i=>-gfrc(t[i],12)*1000);F('FRslope',i=>-gfrSlope(t[i],6)*1000);
  F('Basis',i=>-gB(t[i])*10000);F('BasisZ20',i=>-gBz(t[i],20));F('BasisZ50',i=>-gBz(t[i],50));F('BasisZ100',i=>-gBz(t[i],100));
  F('BasisSlp3',i=>i>=3?-(gB(t[i])-gB(t[i-3]))*10000:0);F('BasisSlp6',i=>i>=6?-(gB(t[i])-gB(t[i-6]))*10000:0);F('BasisSlp12',i=>i>=12?-(gB(t[i])-gB(t[i-12]))*10000:0);
  F('BasisRng',i=>gBrange(t[i]));F('BasisVsFR',i=>(-gB(t[i])*10000)+(-gfr(t[i])*1000));F('FRxBasisZ',i=>(-gfr(t[i])*1000)*(-gBz(t[i],50)));
  F('VolR',i=>{const vs2=sm(v,20);return vs2[i]>0?(v[i]/vs2[i]-1):0;});F('VolSpike',i=>{const vs2=sm(v,50);return vs2[i]>0?Math.log(v[i]/vs2[i]):0;});F('Ret3',i=>i>=3?(c[i]-c[i-3])/c[i-3]*100:0);
...
function engine(sigs,parsed,prng,cfg){
  ...
  for(const sig of sigs){if(cap<=0)break;const d=parsed[sig.pair];const eb=sig.bar+1;if(eb>=d.n)continue;
    for(let i=0;i<cfg.maxPos;i++)advSlot(i,d.t[eb]);
    const busyPairs=new Set(slots.filter(x=>x).map(x=>x.pair));if(busyPairs.has(sig.pair))continue;
    let freeIdx=-1;for(let i=0;i<cfg.maxPos;i++)if(!slots[i]){freeIdx=i;break;}
    if(freeIdx<0)continue;if(prng()>=0.75)continue;if(cap<50)continue;
  ...
  for(let i=0;i<cfg.maxPos;i++){advSlot(i,Infinity);if(slots[i]){const pd=parsed[slots[i].pair];cls(slots[i],pd,Math.min(slots[i].exp,pd.n-1),'TO');}}
...
async function main(){
  console.log('APEX V26 — GRID REFINEMENT around V25 best (PF 1.41 @ 2.4 t/d, $5371)\n');
  ...
  console.log('\n');
  // GRID around V25 best
  const grid=[];
  for(const slM of [2.8,3.0,3.3,3.5,3.8,4.0])
  for(const tpR of [1.5,1.8,2.0,2.2,2.5,3.0])
  for(const fwd of [1,2])
  for(const thrP of [68,72,75,78,82])
  for(const adxF of [22,25,28])
  for(const smartTO of [12,18,24,30])
  for(const fullTO of [48,60,72,84])
  for(const topK of [8,10,12])
  for(const maxPos of [2,3])
  for(const mc of [0.02,0.025,0.03])
    grid.push({slM,tpR,fwd,thrP,adxF,smartTO,fullTO,topK,maxPos,mc,cooldown:1});
  console.log(`Grid size: ${grid.length} — sampling 1200 at random...\n`);
  const rng=P(98765);const shuffled=grid.slice();for(let i=shuffled.length-1;i>0;i--){const j=Math.floor(rng()*(i+1));[shuffled[i],shuffled[j]]=[shuffled[j],shuffled[i]];}
  const toTest=shuffled.slice(0,1200);
  const nW=Math.floor((DAYS-TRAIN_D-TEST_D)/STEP_D)+1;
  let bestPF=0,bestCfg=null,bestTr=[],bestQual=null;
  const top=[];
  for(let att=0;att<toTest.length;att++){
    const cfg=toTest[att];
    ...
    const qual=s.pf*s.pf*Math.min(tpd/2,1.5)*(s.pnl>0?1:0.2);
    top.push({cfg,s,tpd,qual});
    if(s.n>=300&&tpd>=2.0&&s.pf>=1.3&&(!bestQual||qual>bestQual)){bestQual=qual;bestPF=s.pf;bestCfg=cfg;bestTr=allOOS;
      console.log(`[${att}] ${s.n}t ${tpd.toFixed(1)}t/d WR${s.wr.toFixed(1)}% PF${s.pf.toFixed(2)} $${s.pnl.toFixed(0)} qual${qual.toFixed(2)} ◄`);}
    if(att%200===0&&att>0)console.log(`  ... ${att}/${toTest.length}`);}
  if(!bestCfg){console.log('No config met criteria.');return;}
  ...
  console.log('\nTOP 10 configs by quality:');
  top.sort((a,b)=>b.qual-a.qual);for(let i=0;i<Math.min(10,top.length);i++){const t=top[i];console.log(`  #${i+1}: PF ${t.s.pf.toFixed(2)} | ${t.tpd.toFixed(1)}t/d | $${t.s.pnl.toFixed(0)} | q${t.qual.toFixed(2)} | ${JSON.stringify(t.cfg)}`);}
  const lv=s.pf>=1.8&&tpd>=3?'★★★ GOAL REACHED':s.pf>=1.5&&tpd>=2?'★★ STRONG':s.pf>=1.3?'★ COMPETITIVE':'BELOW';
