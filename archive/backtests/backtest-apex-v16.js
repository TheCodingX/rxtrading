...
async function main(){
  const B='═'.repeat(72);
  console.log(`\n${B}\n  APEX V16 — ABSOLUTE MAXIMUM (V14 arch + 3000 configs): Premium Index + Funding + ML\n${B}`);
  ...
  const slMs=[2.0,2.2,2.3,2.5,2.7,3.0,3.5],tpRs=[2.5,2.7,3.0,3.2,3.5,4.0,5.0],fwds=[1,2,3];
  const thrPs=[58,60,62,65,68,70,73,75,78,80],maxPoss=[1,2],timeouts=[48,54,60,66,72,84],adxFs=[20,22,23,25,27];
  let bestPF=0,bestCfg=null,bestTr=[];

  console.log('  Sampling 3000 ULTRA-focused configs with institutional data...\n');
  for(let att=0;att<3000;att++){
    ...
    if(s.pf>bestPF&&s.n>=20){bestPF=s.pf;bestCfg=cfg;bestTr=allOOS;
