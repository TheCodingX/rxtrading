#!/usr/bin/env node
'use strict';
// APEX V10 — 4H HTF + Focused Grid around V6 best + ADX Filter + Multi-Position + Massive Grid Search
...
function get4HTrend(kl4h,barTime){
  if(!kl4h||kl4h.length<25)return 0;
  const cl=kl4h.map(k=>+k[4]),ct=kl4h.map(k=>parseInt(k[6]));
  const e9=em(new Float64Array(cl),9),e21=em(new Float64Array(cl),21);
  let lo2=0,hi2=ct.length-1,b=-1;
  while(lo2<=hi2){const m=(lo2+hi2)>>1;if(ct[m]<=barTime){b=m;lo2=m+1;}else hi2=m-1;}
  return b<21?0:e9[b]>e21[b]?1:-1;
}
async function main(){
  const B='═'.repeat(72);
  console.log(`\n${B}\n  APEX V7 — FOCUSED Grid around V6 best + ADX Filter + Multi-Pos + Massive Grid\n${B}`);
  const startTs=END_TS-DAYS*864e5;const allData={};
  for(const pair of PAIRS){process.stdout.write(`  ${pair}: `);const kl=await gK(pair,'1h',startTs,END_TS);process.stdout.write(`1h=${kl.length} `);const kl4h=await gK(pair,'4h',startTs,END_TS);process.stdout.write(`4h=${kl4h.length} `);const fr=await gF(pair,startTs,END_TS);console.log(`fr=${fr.length}`);allData[pair]={kl,fr,kl4h};}
  ...
  console.log('  Sampling ~500 +4H filter configs from grid...\n');
    ...
    for(let w=0;w<nW&&!skip;w++){
      ...
      for(const pair of PAIRS){
        ...
        for(let i=55;i<d.n-cfg.timeout-1;i++){
          ...
          if(Math.abs(comp)<thr)continue;const tr4h=get4HTrend(allData[pair].kl4h,d.t[i]);if(tr4h!==0&&((comp>0&&tr4h===-1)||(comp<0&&tr4h===1)))continue;
