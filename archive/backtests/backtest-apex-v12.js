#!/usr/bin/env node
'use strict';
// APEX V12 — ULTRA-SELECTIVE + WIDE around V6 best + ADX Filter + Multi-Position + Massive Grid Search
...
async function main(){
  const B='═'.repeat(72);
  console.log(`\n${B}\n  APEX V12 — ULTRA-SELECTIVE + WIDE around V6 best + ADX Filter + Multi-Pos + Massive Grid\n${B}`);
  ...
  const slMs=[0.8,1.0,1.1,1.2,1.3,1.5,1.8,2.0];
  const tpRs=[2.0,2.5,3.0,3.5,4.0,5.0,6.0];
  const fwds=[3,4,5,6];
  const thrPs=[55,58,60,65,70,75,80,85,88,90];
  const trails=[false];
  const maxPoss=[1,2,3,4,5];
  const timeouts=[18,24,30,36,48];
  const adxFilters=[0,18,22,25]; // 0=no filter
  ...
  console.log('  Sampling ~1500 ultra-selective configs from grid...\n');
  ...
  for(let attempt=0;attempt<1500;attempt++){
    const cfg={slM:pick(slMs),tpR:pick(tpRs),fwd:pick(fwds),thrP:pick(thrPs),
      ...
      mc:0.005+rng()*0.015}; // random minCorr 0.008-0.020
