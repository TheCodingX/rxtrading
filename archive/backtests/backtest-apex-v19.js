...
async function main(){
  const B='═'.repeat(72);
  console.log(`\n${B}\n  APEX V19 — V16 config × maxPos × timeout sweep: Premium Index + Funding + ML\n${B}`);
  ...
  console.log('  Testing V16 config with varying maxPos + timeout...\n');
  console.log('  V16 base: sl=2 tp=3 fwd=2 thr=68 adx=22 mc=0.011');
  console.log('  mp | to  | Trades | t/day | WR%    | PF     | PnL      | Mo+  | Pr+');
  console.log('  ' + '-'.repeat(75));
  let bestPF=0,bestCfg=null,bestTr=[];
  const testConfigs=[];
  for(const mp of [1,2,3,4,5,6])for(const to of [24,30,36,48,60,72,84])testConfigs.push({slM:2,tpR:3,fwd:2,thrP:68,maxPos:mp,timeout:to,adxF:22,mc:0.011});
  for(let att=0;att<testConfigs.length;att++){
    const cfg=testConfigs[att];
