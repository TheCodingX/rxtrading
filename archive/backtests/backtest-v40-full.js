#!/usr/bin/env node
'use strict';
// V40 — 30 PAIRS × multi-config walk-forward (no embargo, no maker, market entry)
// GOAL: PF >= 1.4 AND 5-8 t/día (with WR target as secondary)
// Strategy: expand universe max, test multiple stream modes + thresholds + slots
// Walk-forward: same 5 windows (TRAIN 120d / TEST 30d) as AGREE validation
...
async function main(){
  console.log('═'.repeat(80));console.log('V40 — 30 PAIRS walk-forward (TARGET: PF>=1.4, 5-8 t/d)');console.log('═'.repeat(80));
  const allData={};const loaded=[];
  for(const pair of ALL_PAIRS){
    const b1m=load1m(pair);if(!b1m||b1m.length<50000)continue;
    const b1h=aggTF(b1m,60);const b4h=aggTF(b1m,240);
    allData[pair]={b1h,b4h};loaded.push(pair);
  }
  console.log(`LOADED: ${loaded.length}/${ALL_PAIRS.length} pairs\n`);

  const firstTs=Math.min(...Object.values(allData).map(d=>d.b1h[0].t));
  const TRAIN_D=120,TEST_D=30,STEP_D=30;const nW=Math.floor((274-TRAIN_D-TEST_D)/STEP_D)+1;

  const configs=[
    // OR logic: either stream fires independently (1 pair per slot)
    {name:'OR-P45-mp6-30p',mode:'OR',thrP:45,mc:0.011,adxF:22,slM:2,tpR:1.625,to:60,fwd:2,maxPos:6,gate4h:true},
    {name:'OR-P50-mp6-30p',mode:'OR',thrP:50,mc:0.011,adxF:22,slM:2,tpR:1.625,to:60,fwd:2,maxPos:6,gate4h:true},
    {name:'OR-P55-mp6-30p',mode:'OR',thrP:55,mc:0.011,adxF:22,slM:2,tpR:1.625,to:60,fwd:2,maxPos:6,gate4h:true},
    {name:'OR-P60-mp6-30p',mode:'OR',thrP:60,mc:0.011,adxF:22,slM:2,tpR:1.625,to:60,fwd:2,maxPos:6,gate4h:true},
    // M-only mode (momentum only, single stream)
    {name:'M-P45-mp6-30p',mode:'M',thrP:45,mc:0.011,adxF:22,slM:2,tpR:1.625,to:60,fwd:2,maxPos:6,gate4h:true},
    {name:'M-P50-mp6-30p',mode:'M',thrP:50,mc:0.011,adxF:22,slM:2,tpR:1.625,to:60,fwd:2,maxPos:6,gate4h:true},
    {name:'M-P55-mp6-30p',mode:'M',thrP:55,mc:0.011,adxF:22,slM:2,tpR:1.625,to:60,fwd:2,maxPos:6,gate4h:true},
    {name:'M-P60-mp6-30p',mode:'M',thrP:60,mc:0.011,adxF:22,slM:2,tpR:1.625,to:60,fwd:2,maxPos:6,gate4h:true},
    // AGREE (original winner logic scaled)
    {name:'AGREE-P50-mp6-30p',mode:'AGREE',thrP:50,mc:0.011,adxF:22,slM:2,tpR:1.625,to:60,fwd:2,maxPos:6,gate4h:true},
    {name:'AGREE-P55-mp6-30p',mode:'AGREE',thrP:55,mc:0.011,adxF:22,slM:2,tpR:1.625,to:60,fwd:2,maxPos:6,gate4h:true},
    {name:'AGREE-P60-mp6-30p',mode:'AGREE',thrP:60,mc:0.011,adxF:22,slM:2,tpR:1.625,to:60,fwd:2,maxPos:6,gate4h:true},
    {name:'AGREE-P65-mp6-30p',mode:'AGREE',thrP:65,mc:0.011,adxF:22,slM:2,tpR:1.625,to:60,fwd:2,maxPos:6,gate4h:true},
    // Larger maxPos
    {name:'M-P50-mp8-30p',mode:'M',thrP:50,mc:0.011,adxF:22,slM:2,tpR:1.625,to:60,fwd:2,maxPos:8,gate4h:true},
    {name:'OR-P55-mp8-30p',mode:'OR',thrP:55,mc:0.011,adxF:22,slM:2,tpR:1.625,to:60,fwd:2,maxPos:8,gate4h:true},
    {name:'AGREE-P55-mp8-30p',mode:'AGREE',thrP:55,mc:0.011,adxF:22,slM:2,tpR:1.625,to:60,fwd:2,maxPos:8,gate4h:true},
    // Shorter timeout (faster turnover)
    {name:'M-P50-mp6-to36',mode:'M',thrP:50,mc:0.011,adxF:22,slM:2,tpR:1.625,to:36,fwd:2,maxPos:6,gate4h:true},
    {name:'AGREE-P55-mp6-to36',mode:'AGREE',thrP:55,mc:0.011,adxF:22,slM:2,tpR:1.625,to:36,fwd:2,maxPos:6,gate4h:true},
    // No 4H gate
    {name:'M-P50-mp6-noGate',mode:'M',thrP:50,mc:0.011,adxF:22,slM:2,tpR:1.625,to:60,fwd:2,maxPos:6,gate4h:false},
    {name:'AGREE-P55-mp6-noGate',mode:'AGREE',thrP:55,mc:0.011,adxF:22,slM:2,tpR:1.625,to:60,fwd:2,maxPos:6,gate4h:false},
  ];
  ...
  for(const cfg of configs){
    ...
    for(let w=0;w<nW;w++){
      ...
      for(const pair of loaded){
        ...
        const fwd=new Float64Array(Fm.n).fill(NaN);for(let i=50;i<Fm.n-cfg.fwd;i++)fwd[i]=(Fm.c[i+cfg.fwd]-Fm.c[i])/Fm.c[i]*100;
        const coM=pearson(Fm.fs,fwd,50,Fm.n-cfg.fwd);
        const coR=pearson(Fr.fs,fwd,50,Fr.n-cfg.fwd);
      ...
      for(const pair of loaded){
        ...
        for(let i=55;i<Fm.n-cfg.to-1;i++){
          ...
          let finalDir=0;
          if(cfg.mode==='AGREE'){if(passM&&passR&&dirM===dirR)finalDir=dirM;}
          else if(cfg.mode==='M'){if(passM)finalDir=dirM;}
          else if(cfg.mode==='OR'){if(passM&&passR&&dirM===dirR)finalDir=dirM;else if(passM)finalDir=dirM;else if(passR)finalDir=dirR;}
    ...
    const hit=s.pf>=1.4&&tpd>=5&&tpd<=8;
    const partial=s.pf>=1.4&&tpd>=3;
    results.push({cfg,s,tpd,trades:allTrades,hit,partial});
    const marker=hit?' ★★★':(partial?' ★':'');
    console.log(`${cfg.name.padEnd(26)} ${s.n.toString().padStart(4)}t ${tpd.toFixed(2).padStart(5)}t/d WR${s.wr.toFixed(1).padStart(4)}% PF${s.pf.toFixed(2)} Sh${s.sharpe.toFixed(1).padStart(5)} DD$${s.mdd.toFixed(0).padStart(4)} $${s.pnl.toFixed(0).padStart(6)}${marker}`);
  }
  console.log('\n'+'═'.repeat(80));
  const winners=results.filter(r=>r.hit);
  if(winners.length){
    console.log(`★★★ TARGET HIT (PF>=1.4, 5-8 t/d):`);
    winners.sort((a,b)=>b.s.pf-a.s.pf);
    for(const r of winners)console.log(`  ${r.cfg.name}: PF${r.s.pf.toFixed(2)} @ ${r.tpd.toFixed(2)}t/d WR${r.s.wr.toFixed(1)}% Sh${r.s.sharpe.toFixed(1)} $${r.s.pnl.toFixed(0)}`);
    // Best breakdown
    ...
    console.log(`\nBEST: ${best.cfg.name}`);
    console.log('Top 10 pairs by PnL:');for(const{p,s} of pairStats.slice(0,10))console.log(`  ${p.padEnd(12)}: ${s.n}t WR${s.wr.toFixed(1)}% PF${s.pf.toFixed(2)} $${s.pnl.toFixed(0)}`);
    const byMonth={};for(const t of best.trades){const m=t.date.slice(0,7);(byMonth[m]=byMonth[m]||[]).push(t);}
    console.log('\nPer month:');for(const m of Object.keys(byMonth).sort()){const s=st(byMonth[m]);console.log(`  ${m}: ${s.n}t WR${s.wr.toFixed(1)}% PF${s.pf.toFixed(2)} $${s.pnl.toFixed(0)}`);}
  }else{
    const partials=results.filter(r=>r.partial);
    if(partials.length){console.log(`★ PARTIAL (PF>=1.4, 3+ t/d):`);partials.sort((a,b)=>b.s.pf-a.s.pf);for(const r of partials)console.log(`  ${r.cfg.name}: PF${r.s.pf.toFixed(2)} @ ${r.tpd.toFixed(2)}t/d WR${r.s.wr.toFixed(1)}%`);}
    else console.log('✗ No config achieved PF>=1.4 with 3+ t/d');
  }
