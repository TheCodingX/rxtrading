#!/usr/bin/env node
'use strict';
// Train APEX v40 per-pair momentum models for all 30 pairs
// Output: JSON compatible with APEX_V16_MODEL.pairs embedded in app.html
const fs=require('fs');
...
// Same feature names as V16 (24 features) — KEEP ORDER fixed
...
function computeFeatures(bars,fr,piKl){
  ...
  const r14=rsI(c,14),r7=rsI(c,7);
  const stk=stoch(h,l,c,14);
  const bbd=bb(c,20,2);
  const mc=mcd(c);
  const adxR=ax(h,l,c,14);
  ...
  for(let i=50;i<n;i++){
    const atr_i=adxR.atr[i]||1;
    fs[0][i]=(50-r14[i])/50;
    fs[1][i]=(50-r7[i])/50;
    fs[2][i]=isNaN(stk[i])?0:(50-stk[i])/50;
    fs[3][i]=(!isNaN(bbd.up[i])&&bbd.up[i]!==bbd.dn[i])?0.5-(c[i]-bbd.dn[i])/(bbd.up[i]-bbd.dn[i]):0;
    fs[4][i]=mc.hist[i]/atr_i;
    fs[5][i]=i>0?(mc.hist[i]-mc.hist[i-1])/atr_i:0;
    fs[6][i]=(adxR.adx[i]-25)/25;
    fs[7][i]=(e9[i]-e21[i])/atr_i;
    fs[8][i]=(e21[i]-e50[i])/atr_i;
    fs[9][i]=i>=1?(c[i]-c[i-1])/c[i-1]*100:0;
    fs[10][i]=i>=3?(c[i]-c[i-3])/c[i-3]*100:0;
    fs[11][i]=i>=6?(c[i]-c[i-6])/c[i-6]*100:0;
    fs[12][i]=i>=12?(c[i]-c[i-12])/c[i-12]*100:0;
    fs[13][i]=vs[i]>0?(v[i]/vs[i]-1):0;
    fs[14][i]=h[i]!==l[i]?(c[i]-l[i])/(h[i]-l[i])-0.5:0;
    fs[15][i]=-gfr(t[i])*1000;
    fs[16][i]=-gfrc(t[i],3)*1000;
    fs[17][i]=-gfrc(t[i],6)*1000;
    fs[18][i]=-gBasis(t[i])*10000;
    fs[19][i]=-gBasisMA(t[i],24)*10000;
    fs[20][i]=-gBasisZ(t[i]);
    const b1=gBasis(t[i]),b2=i>=6?gBasis(t[i]-6*3600000):b1;
    fs[21][i]=-(b1-b2)*10000;
    fs[22][i]=(-gBasis(t[i])*10000)+(-gfr(t[i])*1000);
    fs[23][i]=Math.sin(new Date(t[i]).getUTCHours()/24*2*Math.PI);
...
async function train(pair){
  const b1m=load1m(pair);
  if(!b1m||b1m.length<50000)return null;
  const b1h=aggTF(b1m,60);
  const fTs=b1h[0].t,lTs=b1h[b1h.length-1].t;
  const fr=await gF(pair,fTs,lTs);
  const pi=await gPI(pair,'1h',fTs,lTs);
  const{features,adx,parsed}=computeFeatures(b1h,fr,pi);
  const n=parsed.c.length;
  // fwd returns 2 bars (same as V16)
  ...
  // Select top 12 features with |corr|>=0.005 (V16 style)
  ...
  // Threshold at P65 (V16 native)
  const tc=[];
  for(let i=55;i<n;i++){if(adx[i]<25)continue;let comp=0;for(const f of sel)comp+=f.corr*(features[f.idx][i]||0);tc.push(Math.abs(comp));}
  tc.sort((a,b)=>a-b);
  const threshold=tc[Math.floor(tc.length*0.65)]||0.001;
  return{
    features:sel.map(x=>({name:x.name,idx:x.idx,corr:x.corr})),
...
async function main(){
  console.log('Training APEX v40 per-pair momentum models...');
  const trained={};let ok=0,fail=0;
  for(const pair of ALL_PAIRS){
    process.stdout.write(pair+' ... ');
    try{
      const m=await train(pair);
      if(m){trained[pair]=m;ok++;console.log(`✓ thr=${m.threshold} feats=${m.count}`);}
      else{fail++;console.log('✗ no valid features');}
    }catch(e){fail++;console.log('✗ '+e.message);}
  }
  console.log(`\nTrained ${ok}/${ALL_PAIRS.length} pairs. Writing output...`);
  // Build config matching APEX_V16_MODEL structure
  const output={
    config:{slM:2,tpR:1.625,fwd:2,thrP:55,maxPos:8,timeout:60,adxF:22,mc:0.011,gate4h:true,dynSize:false,thrScale:0.85,orMode:true,revThresh:0.5},
    features:FEAT_NAMES,
    pairs:trained,
    trainedAt:new Date().toISOString(),
    trainDays:274
  };
  fs.writeFileSync('/tmp/apex-v40-model.json',JSON.stringify(output));
  // Also write compressed single-line version for embedding
  fs.writeFileSync('/tmp/apex-v40-model-inline.json',JSON.stringify(output));
  console.log(`Model written: ${ok} pairs, ${Object.keys(trained).length} entries`);
  console.log(`File size: ${fs.statSync('/tmp/apex-v40-model-inline.json').size} bytes`);
}
